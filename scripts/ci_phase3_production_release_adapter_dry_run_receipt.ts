#!/usr/bin/env node
/**
 * PR #184 — Phase 3 production release adapter dry-run invocation receipt boundary.
 *
 * This harness proves the Gateway records an internal dry-run adapter invocation
 * boundary after the disabled adapter decision is observed.
 *
 * - switch OFF + dry-run OFF => dry-run invocation inactive
 * - switch OFF + dry-run ON  => dry-run invocation inactive
 * - switch ON  + dry-run OFF => dry-run invocation inactive
 * - switch ON  + dry-run ON  => dry-run invocation observed as would_invoke
 * - invocation remains dry-run only
...
const LABEL = "phase3:production-release-adapter-dry-run-receipt-test";

const SWITCH_OFF_GATEWAY_PORT = Number(process.env.PHASE3_PROD_ADAPTER_DRY_RUN_INVOCATION_OFF_PORT || 3108);
const SWITCH_OFF_JWKS_PORT = Number(process.env.PHASE3_PROD_ADAPTER_DRY_RUN_INVOCATION_OFF_JWKS_PORT || 8118);

const SWITCH_ON_GATEWAY_PORT = Number(process.env.PHASE3_PROD_ADAPTER_DRY_RUN_INVOCATION_ON_PORT || 3109);
const SWITCH_ON_JWKS_PORT = Number(process.env.PHASE3_PROD_ADAPTER_DRY_RUN_INVOCATION_ON_JWKS_PORT || 8119);
...
async function main() {
  ...
  const summary = {
    ok: true,
    harness: "phase3.productionReleaseAdapterDryRunReceipt.v1",
