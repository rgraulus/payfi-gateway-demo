...
DIRECT_BUYER_PROOF_PATH="${DIRECT_BUYER_PROOF_PATH:-./.local/phase3/live-direct-buyer/captured-wallet-proof.direct-buyer.local.json}"
...
cleanup() {
  rm -f \
    ...
    "$WORKDIR"/negative-direct-buyer-auth.json \
    ...
    "$WORKDIR"/positive-direct-buyer-auth.json \
...
[[ -f "$DIRECT_BUYER_PROOF_PATH" ]] || fail "Direct Buyer proof file not found at $DIRECT_BUYER_PROOF_PATH. Set DIRECT_BUYER_PROOF_PATH to a local Direct Buyer proof JSON file. This file is intentionally not committed."

echo
echo "============================================================"
echo " x402 v2 Direct Buyer E2E Demo — Engineering Autorun"
...
echo "  Direct Buyer proof file:  $DIRECT_BUYER_PROOF_PATH"
...
curl -fsS "$GW/healthz" >/dev/null || fail "Gateway health check failed at $GW/healthz"
curl -fsS "$CRP/v1/crp/health" >/dev/null || fail "CRP health check failed at $CRP/v1/crp/health"
...
build_direct_buyer_auth() {
  local prefix="$1"
  local region="$2"
  local age_over="$3"

  npm exec -- ts-node --transpile-only scripts/demo_direct_buyer_authorization_proof.ts \
    --payment-required "$WORKDIR/$prefix-gated-pr.json" \
    --proof "$DIRECT_BUYER_PROOF_PATH" \
    --out "$WORKDIR/$prefix-direct-buyer-auth.json" \
    --region "$region" \
    --age-over "$age_over"
}

submit_direct_buyer_auth() {
  local prefix="$1"

  curl -sS -D "$WORKDIR/$prefix-auth-headers.txt" -o "$WORKDIR/$prefix-auth-body.json" \
    -X POST "$GW/paid-gated/redeem" \
    -H 'content-type: application/json' \
    --data-binary @"$WORKDIR/$prefix-direct-buyer-auth.json" >/dev/null

  local status
  status="$(http_status "$WORKDIR/$prefix-auth-headers.txt")"
  echo "$prefix Direct Buyer authorization status: $status"
}
...
build_direct_buyer_auth "negative" "$NEGATIVE_BUYER_REGION" "$NEGATIVE_BUYER_AGE_OVER"
submit_direct_buyer_auth "negative"
...
build_direct_buyer_auth "positive" "$POSITIVE_BUYER_REGION" "$POSITIVE_BUYER_AGE_OVER"
submit_direct_buyer_auth "positive"
...
jq -e '.verifier.ok == true and .verifier.rawProofPrinted == false' "$WORKDIR/positive-auth-body.json" >/dev/null \
  || fail "positive path expected verified Direct Buyer proof without raw proof printing"
...
echo
echo "============================================================"
echo "Final result: x402 v2 Direct Buyer E2E demo complete"
