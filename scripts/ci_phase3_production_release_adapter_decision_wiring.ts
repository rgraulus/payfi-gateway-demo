#!/usr/bin/env node
/**
 * PR #182 — Phase 3 production release adapter disabled-result decision wiring.
 *
 * This harness proves the Gateway decision layer explicitly observes and consumes the
 * PR #181 disabled/no-op adapter result.
 *
 * - switch OFF + dry-run OFF => adapter decision inactive
 * - switch OFF + dry-run ON  => adapter decision inactive
 * - switch ON  + dry-run OFF => adapter decision inactive
 * - switch ON  + dry-run ON  => adapter decision observed as blocked
 * - blocked reason is inherited from the disabled no-op adapter result
 * - adapter decision never allows production release
 * - adapter remains not invoked
...
const LABEL = "phase3:production-release-adapter-decision-wiring-test";

const SWITCH_OFF_GATEWAY_PORT = Number(process.env.PHASE3_PROD_ADAPTER_DECISION_OFF_PORT || 3106);
const SWITCH_OFF_JWKS_PORT = Number(process.env.PHASE3_PROD_ADAPTER_DECISION_OFF_JWKS_PORT || 8116);

const SWITCH_ON_GATEWAY_PORT = Number(process.env.PHASE3_PROD_ADAPTER_DECISION_ON_PORT || 3107);
const SWITCH_ON_JWKS_PORT = Number(process.env.PHASE3_PROD_ADAPTER_DECISION_ON_JWKS_PORT || 8117);
...
async function main() {
  ...
  const summary = {
    ok: true,
    harness: "phase3.productionReleaseAdapterDecisionWiring.v1",
