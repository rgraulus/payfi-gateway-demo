#!/usr/bin/env node
/**
 * PR #201 — Phase 3 production release CRP fulfill client adapter result consumption contract boundary.
 *
 * This harness proves the Gateway observes the result handling gate,
 * then exposes a downstream result consumption contract without consuming the result.
 *
...
const LABEL = "phase3:production-release-crp-fulfill-client-adapter-result-consumption-contract-test";

const SWITCH_OFF_GATEWAY_PORT = Number(process.env.PHASE3_PROD_CRP_FULFILL_CLIENT_ADAPTER_RESULT_CONSUMPTION_CONTRACT_OFF_PORT || 3112);
const SWITCH_OFF_JWKS_PORT = Number(process.env.PHASE3_PROD_CRP_FULFILL_CLIENT_ADAPTER_RESULT_CONSUMPTION_CONTRACT_OFF_JWKS_PORT || 8122);

const SWITCH_ON_GATEWAY_PORT = Number(process.env.PHASE3_PROD_CRP_FULFILL_CLIENT_ADAPTER_RESULT_CONSUMPTION_CONTRACT_ON_PORT || 3113);
const SWITCH_ON_JWKS_PORT = Number(process.env.PHASE3_PROD_CRP_FULFILL_CLIENT_ADAPTER_RESULT_CONSUMPTION_CONTRACT_ON_JWKS_PORT || 8123);
...
async function main() {
  ...
  const summary = {
    ok: true,
    harness: "phase3.productionReleaseCrpFulfillClientAdapterResultConsumptionContract.v1",
