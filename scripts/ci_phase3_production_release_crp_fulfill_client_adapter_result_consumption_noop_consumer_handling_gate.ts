#!/usr/bin/env node
/**
 * PR #210 — Phase 3 production release CRP fulfill client adapter result consumption noop consumer handling gate boundary.
 *
 * This harness proves the Gateway observes the noop consumer, then exposes a downstream
 * handling gate without consuming the result or receipt.
 *
 ...
 * - switch ON  + dry-run ON  + result consumption ON  => noop consumer handling gate observed as blocked and non-consuming
...
const LABEL = "phase3:production-release-crp-fulfill-client-adapter-result-consumption-noop-consumer-handling-gate-test";

const SWITCH_OFF_GATEWAY_PORT = Number(process.env.PHASE3_PROD_CRP_FULFILL_CLIENT_ADAPTER_RESULT_CONSUMPTION_NOOP_CONSUMER_HANDLING_GATE_OFF_PORT || 3130);
const SWITCH_OFF_JWKS_PORT = Number(process.env.PHASE3_PROD_CRP_FULFILL_CLIENT_ADAPTER_RESULT_CONSUMPTION_NOOP_CONSUMER_HANDLING_GATE_OFF_JWKS_PORT || 8140);

const SWITCH_ON_GATEWAY_PORT = Number(process.env.PHASE3_PROD_CRP_FULFILL_CLIENT_ADAPTER_RESULT_CONSUMPTION_NOOP_CONSUMER_HANDLING_GATE_ON_PORT || 3131);
const SWITCH_ON_JWKS_PORT = Number(process.env.PHASE3_PROD_CRP_FULFILL_CLIENT_ADAPTER_RESULT_CONSUMPTION_NOOP_CONSUMER_HANDLING_GATE_ON_JWKS_PORT || 8141);
...
async function main() {
  ...
  const summary = {
    ok: true,
    harness: "phase3.productionReleaseCrpFulfillClientAdapterResultConsumptionNoopConsumerHandlingGate.v1",
