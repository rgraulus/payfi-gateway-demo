#!/usr/bin/env node
/**
 * scripts/ci_phase3_live_direct_buyer_receipt_decoder_adapter_test_only_invocation_seam_preflight.ts
 *
 * Milestone 3H / PR #238 harness.
 *
 ...
 * a test-only adapter-call contract preflight, and a test-only
 * invocation-seam preflight bound to the fresh PAYMENT-REQUIRED tuple.
 *
 ...
 * does not invoke decode, proves the adapter invocation seam can be crossed under test-only control without real decode,
...
const GATEWAY_PORT = Number(process.env.PHASE3_LIVE_DIRECT_BUYER_RECEIPT_DECODER_ADAPTER_TEST_ONLY_INVOCATION_SEAM_PREFLIGHT_PORT || 3105);
const JWKS_PORT = Number(process.env.PHASE3_LIVE_DIRECT_BUYER_RECEIPT_DECODER_ADAPTER_TEST_ONLY_INVOCATION_SEAM_PREFLIGHT_JWKS_PORT || 8155);
const BASE = baseUrlForPort(GATEWAY_PORT);
const JWKS_BASE = baseUrlForPort(JWKS_PORT);
const LABEL = "phase3:live-direct-buyer-receipt-decoder-adapter-test-only-invocation-seam-preflight-test";
    ...
    JSON.stringify(
      {
        ...
        code: "usage",
        reason:
          "Usage: PHASE3_LIVE_DIRECT_BUYER_RECEIPT_DECODER_ADAPTER_TEST_ONLY_INVOCATION_SEAM_PREFLIGHT_HARNESS=true npm run phase3:live-direct-buyer-receipt-decoder-adapter-test-only-invocation-seam-preflight-test -- <local-wallet-proof.json>",
...
async function main() {
  ...
  if (String(process.env.PHASE3_LIVE_DIRECT_BUYER_RECEIPT_DECODER_ADAPTER_TEST_ONLY_INVOCATION_SEAM_PREFLIGHT_HARNESS ?? "").toLowerCase() !== "true") {
    console.error(
      JSON.stringify(
        {
          ok: false,
          ...
          reason:
            "Set PHASE3_LIVE_DIRECT_BUYER_RECEIPT_DECODER_ADAPTER_TEST_ONLY_INVOCATION_SEAM_PREFLIGHT_HARNESS=true to run this local-only harness.",
          rawProofPrinted: false,
          rawReceiptPrinted: false,
        },
        null,
  ...
  try {
    ...
    console.log(
      JSON.stringify(
        {
          ok: true,
          harness: "phase3.liveDirectBuyerReceiptDecoderAdapterTestOnlyInvocationSeamPreflight.v1",
