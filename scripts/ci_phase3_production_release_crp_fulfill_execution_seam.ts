#!/usr/bin/env node
/**
 * PR #187 — Phase 3 production release CRP fulfill execution seam boundary.
 *
 * This harness proves the Gateway recognizes the validated CRP fulfill request
 * as ready for the future execution seam, while keeping that seam hard-disabled.
 *
...
const LABEL = "phase3:production-release-crp-fulfill-execution-seam-test";

const SWITCH_OFF_GATEWAY_PORT = Number(process.env.PHASE3_PROD_CRP_FULFILL_EXECUTION_SEAM_OFF_PORT || 3112);
const SWITCH_OFF_JWKS_PORT = Number(process.env.PHASE3_PROD_CRP_FULFILL_EXECUTION_SEAM_OFF_JWKS_PORT || 8122);

const SWITCH_ON_GATEWAY_PORT = Number(process.env.PHASE3_PROD_CRP_FULFILL_EXECUTION_SEAM_ON_PORT || 3113);
const SWITCH_ON_JWKS_PORT = Number(process.env.PHASE3_PROD_CRP_FULFILL_EXECUTION_SEAM_ON_JWKS_PORT || 8123);
...
async function main() {
  ...
  const summary = {
    ok: true,
    harness: "phase3.productionReleaseCrpFulfillExecutionSeam.v1",
