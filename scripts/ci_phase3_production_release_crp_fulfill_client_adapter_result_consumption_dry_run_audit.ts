#!/usr/bin/env node
/**
 * PR #205 — Phase 3 production release CRP fulfill client adapter result consumption dry-run audit artifact boundary.
 *
 * This harness proves the Gateway observes the blocked result consumption handling gate,
 * then emits a downstream sanitized dry-run audit artifact without consuming the result.
 *
 * - switch OFF + dry-run OFF => dry-run audit inactive
 * - switch OFF + dry-run ON  => dry-run audit inactive
 * - switch ON  + dry-run OFF => dry-run audit inactive
 * - switch ON  + dry-run ON  => dry-run audit recorded after handling gate
 * - no result or receipt is consumed
...
const LABEL = "phase3:production-release-crp-fulfill-client-adapter-result-consumption-dry-run-audit-test";

const SWITCH_OFF_GATEWAY_PORT = Number(process.env.PHASE3_PROD_CRP_FULFILL_CLIENT_ADAPTER_RESULT_CONSUMPTION_DRY_RUN_AUDIT_OFF_PORT || 3120);
const SWITCH_OFF_JWKS_PORT = Number(process.env.PHASE3_PROD_CRP_FULFILL_CLIENT_ADAPTER_RESULT_CONSUMPTION_DRY_RUN_AUDIT_OFF_JWKS_PORT || 8130);

const SWITCH_ON_GATEWAY_PORT = Number(process.env.PHASE3_PROD_CRP_FULFILL_CLIENT_ADAPTER_RESULT_CONSUMPTION_DRY_RUN_AUDIT_ON_PORT || 3121);
const SWITCH_ON_JWKS_PORT = Number(process.env.PHASE3_PROD_CRP_FULFILL_CLIENT_ADAPTER_RESULT_CONSUMPTION_DRY_RUN_AUDIT_ON_JWKS_PORT || 8131);
...
async function main() {
  ...
  const summary = {
    ok: true,
    harness: "phase3.productionReleaseCrpFulfillClientAdapterResultConsumptionDryRunAudit.v1",
