#!/usr/bin/env node
/**
 * scripts/ci_phase3_live_direct_buyer_receipt_decoder_noop_disabled_invocation_guard.ts
 *
 * Milestone 3G / PR #231 harness.
 *
 ...
 * a decoder-input contract descriptor plus a disabled/noop decoder invocation
 * guard bound to the fresh PAYMENT-REQUIRED tuple.
 *
 ...
 * does not invoke decode, proves decoder invocation remains disabled/noop,
 * does not verify the receipt in the runtime path, does not release the protected
...
const GATEWAY_PORT = Number(process.env.PHASE3_LIVE_DIRECT_BUYER_RECEIPT_DECODER_NOOP_DISABLED_INVOCATION_GUARD_PORT || 3104);
const JWKS_PORT = Number(process.env.PHASE3_LIVE_DIRECT_BUYER_RECEIPT_DECODER_NOOP_DISABLED_INVOCATION_GUARD_JWKS_PORT || 8154);
const BASE = baseUrlForPort(GATEWAY_PORT);
const JWKS_BASE = baseUrlForPort(JWKS_PORT);
const LABEL = "phase3:live-direct-buyer-receipt-decoder-noop-disabled-invocation-guard-test";
    ...
    JSON.stringify(
      {
        ...
        code: "usage",
        reason:
          "Usage: PHASE3_LIVE_DIRECT_BUYER_RECEIPT_DECODER_NOOP_DISABLED_INVOCATION_GUARD_HARNESS=true npm run phase3:live-direct-buyer-receipt-decoder-noop-disabled-invocation-guard-test -- <local-wallet-proof.json>",
...
function buildSanitizedReceiptJwsHandoff(input: {
  ...
  return {
    contract: "phase3.liveDirectBuyer.receiptDecoderNoopDisabledInvocationGuard.v1",
    mode: "preflight_only",
    status: "handoff_ready",
    reason: "receipt_decoder_invocation_guard_proved_disabled_noop_without_runtime_decoder_invocation",
    source: {
    ...
    decoderInputContract: (() => {
      ...
      return {
        ...
        inputShape: {
          ...
          decoderContractVersion: "phase3.liveDirectBuyer.receiptDecoderNoopDisabledInvocationGuard.v1",
        },
        allowedMetadataCategories: [...descriptor.decoderInput.allowedMetadataCategories],
        prohibitedReceiptMaterialCategories: [...descriptor.decoderInput.prohibitedReceiptMaterialCategories],
      };
...
async function main() {
  ...
  if (String(process.env.PHASE3_LIVE_DIRECT_BUYER_RECEIPT_DECODER_NOOP_DISABLED_INVOCATION_GUARD_HARNESS ?? "").toLowerCase() !== "true") {
    console.error(
      JSON.stringify(
        {
          ok: false,
          ...
          reason:
            "Set PHASE3_LIVE_DIRECT_BUYER_RECEIPT_DECODER_NOOP_DISABLED_INVOCATION_GUARD_HARNESS=true to run this local-only harness.",
          rawProofPrinted: false,
          rawReceiptPrinted: false,
        },
        null,
  ...
  try {
    ...
    assert.equal(handoff.decoderInputContract.contract, "phase3.liveDirectBuyer.receiptDecoderNoopDisabledInvocationGuard.v1");
    ...
    assert.equal(handoff.decoderInvocationGuard.contract, "phase3.liveDirectBuyer.receiptDecoderNoopDisabledInvocationGuard.v1");
    ...
    console.log(
      JSON.stringify(
        {
          ok: true,
          harness: "phase3.liveDirectBuyerReceiptDecoderNoopDisabledInvocationGuard.v1",
          ...
          decoderNoopDisabledInvocationGuardOnly: true,
