#!/usr/bin/env node
/**
 * PR #203 — Phase 3 production release CRP fulfill client adapter result consumption noop result boundary.
 *
 * This harness proves the Gateway observes the result consumption decision gate,
 * then emits a disabled result consumption noop result without consuming the result.
 *
...
const LABEL = "phase3:production-release-crp-fulfill-client-adapter-result-consumption-noop-result-test";

const SWITCH_OFF_GATEWAY_PORT = Number(process.env.PHASE3_PROD_CRP_FULFILL_CLIENT_ADAPTER_RESULT_CONSUMPTION_NOOP_RESULT_OFF_PORT || 3116);
const SWITCH_OFF_JWKS_PORT = Number(process.env.PHASE3_PROD_CRP_FULFILL_CLIENT_ADAPTER_RESULT_CONSUMPTION_NOOP_RESULT_OFF_JWKS_PORT || 8126);

const SWITCH_ON_GATEWAY_PORT = Number(process.env.PHASE3_PROD_CRP_FULFILL_CLIENT_ADAPTER_RESULT_CONSUMPTION_NOOP_RESULT_ON_PORT || 3117);
const SWITCH_ON_JWKS_PORT = Number(process.env.PHASE3_PROD_CRP_FULFILL_CLIENT_ADAPTER_RESULT_CONSUMPTION_NOOP_RESULT_ON_JWKS_PORT || 8127);
...
async function main() {
  ...
  const summary = {
    ok: true,
    harness: "phase3.productionReleaseCrpFulfillClientAdapterResultConsumptionNoopResult.v1",
