#!/usr/bin/env node
/**
 * PR #188 — Phase 3 production release CRP fulfill client adapter scaffold boundary.
 *
 * This harness proves the Gateway recognizes the disabled CRP fulfill execution seam
 * as ready for the future client adapter scaffold, while keeping that scaffold hard-disabled.
 *
...
const LABEL = "phase3:production-release-crp-fulfill-client-adapter-scaffold-test";

const SWITCH_OFF_GATEWAY_PORT = Number(process.env.PHASE3_PROD_CRP_FULFILL_CLIENT_ADAPTER_SCAFFOLD_OFF_PORT || 3112);
const SWITCH_OFF_JWKS_PORT = Number(process.env.PHASE3_PROD_CRP_FULFILL_CLIENT_ADAPTER_SCAFFOLD_OFF_JWKS_PORT || 8122);

const SWITCH_ON_GATEWAY_PORT = Number(process.env.PHASE3_PROD_CRP_FULFILL_CLIENT_ADAPTER_SCAFFOLD_ON_PORT || 3113);
const SWITCH_ON_JWKS_PORT = Number(process.env.PHASE3_PROD_CRP_FULFILL_CLIENT_ADAPTER_SCAFFOLD_ON_JWKS_PORT || 8123);
...
async function main() {
  ...
  const summary = {
    ok: true,
    harness: "phase3.productionReleaseCrpFulfillClientAdapterScaffold.v1",
