...
function startGateway() {
  ...
  child.on("error", (err) => {
    console.error("[phase3:gateway-x402-receipt-composition-test] gateway spawn error:", err);
  });
...
async function main() {
  console.log(`[phase3:gateway-x402-receipt-composition-test] BASE=${BASE}`);
  ...
  try {
    ...
    assert.equal(stillNoRelease.status, 402, "x402 receipt composition test must not release resource");
...
main().catch((err) => {
  console.error("[phase3:gateway-x402-receipt-composition-test] ERROR:", err?.stack || err?.message || err);
  process.exit(1);
});
