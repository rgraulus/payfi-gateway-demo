#!/usr/bin/env node
/**
 * PR #209 — Phase 3 production release CRP fulfill client adapter result consumption noop consumer boundary.
 *
 * This harness proves the Gateway observes the activation preflight, then exposes a downstream
 * noop consumer boundary without consuming the result or receipt.
 *
 ...
 * - switch ON  + dry-run ON  + result consumption ON  => noop consumer observed as disabled and non-consuming
...
const LABEL = "phase3:production-release-crp-fulfill-client-adapter-result-consumption-noop-consumer-test";

const SWITCH_OFF_GATEWAY_PORT = Number(process.env.PHASE3_PROD_CRP_FULFILL_CLIENT_ADAPTER_RESULT_CONSUMPTION_NOOP_CONSUMER_OFF_PORT || 3128);
const SWITCH_OFF_JWKS_PORT = Number(process.env.PHASE3_PROD_CRP_FULFILL_CLIENT_ADAPTER_RESULT_CONSUMPTION_NOOP_CONSUMER_OFF_JWKS_PORT || 8138);

const SWITCH_ON_GATEWAY_PORT = Number(process.env.PHASE3_PROD_CRP_FULFILL_CLIENT_ADAPTER_RESULT_CONSUMPTION_NOOP_CONSUMER_ON_PORT || 3129);
const SWITCH_ON_JWKS_PORT = Number(process.env.PHASE3_PROD_CRP_FULFILL_CLIENT_ADAPTER_RESULT_CONSUMPTION_NOOP_CONSUMER_ON_JWKS_PORT || 8139);
...
async function main() {
  ...
  const summary = {
    ok: true,
    harness: "phase3.productionReleaseCrpFulfillClientAdapterResultConsumptionNoopConsumer.v1",
