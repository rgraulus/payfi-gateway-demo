#!/usr/bin/env node
/**
 * PR #202 — Phase 3 production release CRP fulfill client adapter result consumption decision gate boundary.
 *
 * This harness proves the Gateway observes the result consumption contract,
 * then exposes a downstream result consumption decision gate without consuming the result.
 *
...
const LABEL = "phase3:production-release-crp-fulfill-client-adapter-result-consumption-decision-gate-test";

const SWITCH_OFF_GATEWAY_PORT = Number(process.env.PHASE3_PROD_CRP_FULFILL_CLIENT_ADAPTER_RESULT_CONSUMPTION_DECISION_GATE_OFF_PORT || 3114);
const SWITCH_OFF_JWKS_PORT = Number(process.env.PHASE3_PROD_CRP_FULFILL_CLIENT_ADAPTER_RESULT_CONSUMPTION_DECISION_GATE_OFF_JWKS_PORT || 8124);

const SWITCH_ON_GATEWAY_PORT = Number(process.env.PHASE3_PROD_CRP_FULFILL_CLIENT_ADAPTER_RESULT_CONSUMPTION_DECISION_GATE_ON_PORT || 3115);
const SWITCH_ON_JWKS_PORT = Number(process.env.PHASE3_PROD_CRP_FULFILL_CLIENT_ADAPTER_RESULT_CONSUMPTION_DECISION_GATE_ON_JWKS_PORT || 8125);
...
async function main() {
  ...
  const summary = {
    ok: true,
    harness: "phase3.productionReleaseCrpFulfillClientAdapterResultConsumptionDecisionGate.v1",
