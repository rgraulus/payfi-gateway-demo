#!/usr/bin/env node
/**
 * PR 4-7 — Phase 4 real receipt release eligibility composition preflight.
 *
 * Receipt release eligibility composition preflight checkpoint.
 *
 * This validates the Phase 4 receipt release eligibility composition preflight on top of the tuple binding verification preflight:
...
const LABEL = "phase4:real-receipt-release-eligibility-composition-preflight-test";
const GATEWAY_PORT = Number(process.env.PHASE4_REAL_RECEIPT_RELEASE_ELIGIBILITY_COMPOSITION_PREFLIGHT_PORT || 3128);
const CRP_PORT = Number(process.env.PHASE4_REAL_RECEIPT_RELEASE_ELIGIBILITY_COMPOSITION_PREFLIGHT_CRP_PORT || 8128);
...
async function syntheticReceiptJws(body: any, privateKey: any): Promise<string> {
  ...
  return await new SignJWT({
    ...
    sub: "phase4-real-receipt-release-eligibility-composition-preflight",
    receiptVersion: "phase4.synthetic.release-eligibility-composition-preflight.v1",
...
async function main() {
  ...
  try {
    ...
    assert.equal(decodePreflight?.payload?.receiptVersion, "phase4.synthetic.release-eligibility-composition-preflight.v1");
    ...
    assert.equal(signaturePreflight?.payload?.receiptVersion, "phase4.synthetic.release-eligibility-composition-preflight.v1");
