#!/usr/bin/env node
/**
 * PR 4-3 — Phase 4 real receipt JWS decode preflight.
 *
 * Receipt JWS decode preflight checkpoint.
 *
 * This validates the Phase 4 receipt JWS decode preflight on top of the handoff contract:
...
const LABEL = "phase4:real-receipt-jws-decode-preflight-test";
const GATEWAY_PORT = Number(process.env.PHASE4_REAL_RECEIPT_JWS_DECODE_PREFLIGHT_PORT || 3124);
const CRP_PORT = Number(process.env.PHASE4_REAL_RECEIPT_JWS_DECODE_PREFLIGHT_CRP_PORT || 8124);
...
function syntheticReceiptJws(body: any): string {
  const header = b64urlJson({
    alg: "EdDSA",
    typ: "JWT",
    kid: "phase4-test-only",
  });

  const payload = b64urlJson({
    iss: "phase4-test-crp",
    aud: "payfi-gateway-demo",
    sub: "phase4-real-receipt-jws-decode-preflight",
    receiptVersion: "phase4.synthetic.decode-preflight.v1",
...
async function main() {
  ...
  try {
    ...
    assert.equal(decodePreflight?.payload?.receiptVersion, "phase4.synthetic.decode-preflight.v1");
