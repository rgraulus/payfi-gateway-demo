#!/usr/bin/env node
/**
 * PR #206 — Phase 3 production release CRP fulfill client adapter result consumption readiness gate boundary.
 *
 * This harness proves the Gateway observes the sanitized dry-run audit artifact,
 * then exposes a downstream readiness gate without consuming the result or receipt.
 *
 ...
 * - switch ON  + dry-run ON  => readiness gate observed as blocked after dry-run audit
...
const LABEL = "phase3:production-release-crp-fulfill-client-adapter-result-consumption-readiness-gate-test";

const SWITCH_OFF_GATEWAY_PORT = Number(process.env.PHASE3_PROD_CRP_FULFILL_CLIENT_ADAPTER_RESULT_CONSUMPTION_READINESS_GATE_OFF_PORT || 3122);
const SWITCH_OFF_JWKS_PORT = Number(process.env.PHASE3_PROD_CRP_FULFILL_CLIENT_ADAPTER_RESULT_CONSUMPTION_READINESS_GATE_OFF_JWKS_PORT || 8132);

const SWITCH_ON_GATEWAY_PORT = Number(process.env.PHASE3_PROD_CRP_FULFILL_CLIENT_ADAPTER_RESULT_CONSUMPTION_READINESS_GATE_ON_PORT || 3123);
const SWITCH_ON_JWKS_PORT = Number(process.env.PHASE3_PROD_CRP_FULFILL_CLIENT_ADAPTER_RESULT_CONSUMPTION_READINESS_GATE_ON_JWKS_PORT || 8133);
...
async function main() {
  ...
  const summary = {
    ok: true,
    harness: "phase3.productionReleaseCrpFulfillClientAdapterResultConsumptionReadinessGate.v1",
