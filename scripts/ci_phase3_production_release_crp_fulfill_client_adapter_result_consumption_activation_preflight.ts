#!/usr/bin/env node
/**
 * PR #208 — Phase 3 production release CRP fulfill client adapter result consumption activation preflight boundary.
 *
 * This harness proves the Gateway observes the enablement gate, then exposes a downstream
 * activation preflight without consuming the result or receipt.
 *
 ...
 * - switch ON  + dry-run ON  + result consumption ON  => activation preflight observed as ready but non-consuming
...
const LABEL = "phase3:production-release-crp-fulfill-client-adapter-result-consumption-activation-preflight-test";

const SWITCH_OFF_GATEWAY_PORT = Number(process.env.PHASE3_PROD_CRP_FULFILL_CLIENT_ADAPTER_RESULT_CONSUMPTION_ACTIVATION_PREFLIGHT_OFF_PORT || 3126);
const SWITCH_OFF_JWKS_PORT = Number(process.env.PHASE3_PROD_CRP_FULFILL_CLIENT_ADAPTER_RESULT_CONSUMPTION_ACTIVATION_PREFLIGHT_OFF_JWKS_PORT || 8136);

const SWITCH_ON_GATEWAY_PORT = Number(process.env.PHASE3_PROD_CRP_FULFILL_CLIENT_ADAPTER_RESULT_CONSUMPTION_ACTIVATION_PREFLIGHT_ON_PORT || 3127);
const SWITCH_ON_JWKS_PORT = Number(process.env.PHASE3_PROD_CRP_FULFILL_CLIENT_ADAPTER_RESULT_CONSUMPTION_ACTIVATION_PREFLIGHT_ON_JWKS_PORT || 8137);
...
async function main() {
  ...
  const summary = {
    ok: true,
    harness: "phase3.productionReleaseCrpFulfillClientAdapterResultConsumptionActivationPreflight.v1",
