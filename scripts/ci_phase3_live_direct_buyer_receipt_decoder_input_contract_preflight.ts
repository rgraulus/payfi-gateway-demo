#!/usr/bin/env node
/**
 * scripts/ci_phase3_live_direct_buyer_receipt_decoder_input_contract_preflight.ts
 *
 * Milestone 3G / PR #230 harness.
 *
 ...
 * and a decoder-input contract descriptor bound to the fresh PAYMENT-REQUIRED tuple.
 *
 * This is intentionally a guarded local preflight harness. It does not submit
 * the receipt JWS to Gateway, does not build a runtime decoder input object, does not invoke decode, does
 * not verify the receipt in the runtime path, does not release the protected
...
const GATEWAY_PORT = Number(process.env.PHASE3_LIVE_DIRECT_BUYER_RECEIPT_DECODER_INPUT_CONTRACT_PREFLIGHT_PORT || 3103);
const JWKS_PORT = Number(process.env.PHASE3_LIVE_DIRECT_BUYER_RECEIPT_DECODER_INPUT_CONTRACT_PREFLIGHT_JWKS_PORT || 8153);
const BASE = baseUrlForPort(GATEWAY_PORT);
const JWKS_BASE = baseUrlForPort(JWKS_PORT);
const LABEL = "phase3:live-direct-buyer-receipt-decoder-input-contract-preflight-test";
    ...
    JSON.stringify(
      {
        ...
        code: "usage",
        reason:
          "Usage: PHASE3_LIVE_DIRECT_BUYER_RECEIPT_DECODER_INPUT_CONTRACT_PREFLIGHT_HARNESS=true npm run phase3:live-direct-buyer-receipt-decoder-input-contract-preflight-test -- <local-wallet-proof.json>",
...
function buildSanitizedReceiptJwsHandoff(input: {
  ...
  return {
    contract: "phase3.liveDirectBuyer.receiptDecoderInputContractPreflight.v1",
    mode: "preflight_only",
    status: "handoff_ready",
    reason: "receipt_decoder_input_contract_built_without_runtime_decoder_invocation",
    source: {
    ...
    decoderInputContract: (() => {
      ...
      return {
        ...
        inputShape: {
          ...
          decoderContractVersion: "phase3.liveDirectBuyer.receiptDecoderInputContractPreflight.v1",
        },
        allowedMetadataCategories: [...descriptor.decoderInput.allowedMetadataCategories],
        prohibitedReceiptMaterialCategories: [...descriptor.decoderInput.prohibitedReceiptMaterialCategories],
      };
...
async function main() {
  ...
  if (String(process.env.PHASE3_LIVE_DIRECT_BUYER_RECEIPT_DECODER_INPUT_CONTRACT_PREFLIGHT_HARNESS ?? "").toLowerCase() !== "true") {
    console.error(
      JSON.stringify(
        {
          ok: false,
          ...
          reason:
            "Set PHASE3_LIVE_DIRECT_BUYER_RECEIPT_DECODER_INPUT_CONTRACT_PREFLIGHT_HARNESS=true to run this local-only harness.",
          rawProofPrinted: false,
          rawReceiptPrinted: false,
        },
        null,
  ...
  try {
    ...
    assert.equal(handoff.decoderInputContract.contract, "phase3.liveDirectBuyer.receiptDecoderInputContractPreflight.v1");
    ...
    console.log(
      JSON.stringify(
        {
          ok: true,
          harness: "phase3.liveDirectBuyerReceiptDecoderInputContractPreflight.v1",
