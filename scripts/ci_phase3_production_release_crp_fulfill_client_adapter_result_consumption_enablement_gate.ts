#!/usr/bin/env node
/**
 * PR #207 — Phase 3 production release CRP fulfill client adapter result consumption enablement gate boundary.
 *
 * This harness proves the Gateway observes the readiness gate, then exposes a downstream
 * enablement gate without consuming the result or receipt.
 *
 ...
 * - switch ON  + dry-run ON  + result consumption ON  => enablement gate observed as enabled but non-consuming
...
const LABEL = "phase3:production-release-crp-fulfill-client-adapter-result-consumption-enablement-gate-test";

const SWITCH_OFF_GATEWAY_PORT = Number(process.env.PHASE3_PROD_CRP_FULFILL_CLIENT_ADAPTER_RESULT_CONSUMPTION_ENABLEMENT_GATE_OFF_PORT || 3124);
const SWITCH_OFF_JWKS_PORT = Number(process.env.PHASE3_PROD_CRP_FULFILL_CLIENT_ADAPTER_RESULT_CONSUMPTION_ENABLEMENT_GATE_OFF_JWKS_PORT || 8134);

const SWITCH_ON_GATEWAY_PORT = Number(process.env.PHASE3_PROD_CRP_FULFILL_CLIENT_ADAPTER_RESULT_CONSUMPTION_ENABLEMENT_GATE_ON_PORT || 3125);
const SWITCH_ON_JWKS_PORT = Number(process.env.PHASE3_PROD_CRP_FULFILL_CLIENT_ADAPTER_RESULT_CONSUMPTION_ENABLEMENT_GATE_ON_JWKS_PORT || 8135);
...
async function main() {
  ...
  const summary = {
    ok: true,
    harness: "phase3.productionReleaseCrpFulfillClientAdapterResultConsumptionEnablementGate.v1",
