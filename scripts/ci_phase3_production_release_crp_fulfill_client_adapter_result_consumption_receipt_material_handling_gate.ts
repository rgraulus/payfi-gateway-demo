#!/usr/bin/env node
/**
 * PR #212 — Phase 3 production release CRP fulfill client adapter result consumption receipt material handling gate.
 *
 * This harness proves the Gateway observes the receipt material preflight, then exposes
 * a receipt material handling gate that remains blocked and non-consuming.
 *
 ...
 * - switch ON  + dry-run ON  + result consumption ON  => receipt material handling gate observed as blocked and non-consuming
...
const LABEL = "phase3:production-release-crp-fulfill-client-adapter-result-consumption-receipt-material-handling-gate-test";

const SWITCH_OFF_GATEWAY_PORT = Number(process.env.PHASE3_PROD_CRP_FULFILL_CLIENT_ADAPTER_RESULT_CONSUMPTION_RECEIPT_MATERIAL_HANDLING_GATE_OFF_PORT || 3134);
const SWITCH_OFF_JWKS_PORT = Number(process.env.PHASE3_PROD_CRP_FULFILL_CLIENT_ADAPTER_RESULT_CONSUMPTION_RECEIPT_MATERIAL_HANDLING_GATE_OFF_JWKS_PORT || 8144);

const SWITCH_ON_GATEWAY_PORT = Number(process.env.PHASE3_PROD_CRP_FULFILL_CLIENT_ADAPTER_RESULT_CONSUMPTION_RECEIPT_MATERIAL_HANDLING_GATE_ON_PORT || 3135);
const SWITCH_ON_JWKS_PORT = Number(process.env.PHASE3_PROD_CRP_FULFILL_CLIENT_ADAPTER_RESULT_CONSUMPTION_RECEIPT_MATERIAL_HANDLING_GATE_ON_JWKS_PORT || 8145);
...
async function main() {
  ...
  const summary = {
    ok: true,
    harness: "phase3.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptMaterialHandlingGate.v1",
