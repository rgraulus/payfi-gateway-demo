#!/usr/bin/env node
/**
 * PR 4-8 — Phase 4 real receipt replay + canonical release persistence preflight.
 *
 * Receipt replay + canonical release persistence preflight checkpoint.
 *
 * This validates the Phase 4 receipt replay + canonical release persistence preflight on top of the release eligibility composition preflight:
...
const LABEL = "phase4:real-receipt-replay-canonical-persistence-preflight-test";
const GATEWAY_PORT = Number(process.env.PHASE4_REAL_RECEIPT_REPLAY_CANONICAL_PERSISTENCE_PREFLIGHT_PORT || 3129);
const CRP_PORT = Number(process.env.PHASE4_REAL_RECEIPT_REPLAY_CANONICAL_PERSISTENCE_PREFLIGHT_CRP_PORT || 8129);
...
async function syntheticReceiptJws(body: any, privateKey: any): Promise<string> {
  ...
  return await new SignJWT({
    ...
    sub: "phase4-real-receipt-replay-canonical-persistence-preflight",
    receiptVersion: "phase4.synthetic.replay-canonical-persistence-preflight.v1",
...
async function main() {
  ...
  try {
    ...
    assert.equal(decodePreflight?.payload?.receiptVersion, "phase4.synthetic.replay-canonical-persistence-preflight.v1");
    ...
    assert.equal(signaturePreflight?.payload?.receiptVersion, "phase4.synthetic.replay-canonical-persistence-preflight.v1");
