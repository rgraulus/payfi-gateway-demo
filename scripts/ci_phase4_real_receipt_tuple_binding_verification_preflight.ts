#!/usr/bin/env node
/**
 * PR 4-6 — Phase 4 real receipt tuple binding verification preflight.
 *
 * Receipt tuple binding verification preflight checkpoint.
 *
 * This validates the Phase 4 receipt tuple binding verification preflight on top of the settlement verification preflight:
...
const LABEL = "phase4:real-receipt-tuple-binding-verification-preflight-test";
const GATEWAY_PORT = Number(process.env.PHASE4_REAL_RECEIPT_TUPLE_BINDING_VERIFICATION_PREFLIGHT_PORT || 3127);
const CRP_PORT = Number(process.env.PHASE4_REAL_RECEIPT_TUPLE_BINDING_VERIFICATION_PREFLIGHT_CRP_PORT || 8127);
...
async function syntheticReceiptJws(body: any, privateKey: any): Promise<string> {
  ...
  return await new SignJWT({
    ...
    sub: "phase4-real-receipt-tuple-binding-verification-preflight",
    receiptVersion: "phase4.synthetic.tuple-binding-verification-preflight.v1",
...
async function main() {
  ...
  try {
    ...
    assert.equal(decodePreflight?.payload?.receiptVersion, "phase4.synthetic.tuple-binding-verification-preflight.v1");
    ...
    assert.equal(signaturePreflight?.payload?.receiptVersion, "phase4.synthetic.tuple-binding-verification-preflight.v1");
