#!/usr/bin/env node
/**
 * PR #180 — Phase 3 production release adapter input builder.
 *
 * This harness proves the sanitized adapter input boundary after dry-run would-execute.
 *
 * - switch OFF + dry-run OFF => adapter input is not built
 * - switch OFF + dry-run ON  => adapter input is not built
 * - switch ON  + dry-run OFF => adapter input is not built
 * - switch ON  + dry-run ON  => sanitized adapter input is built and ready
...
const LABEL = "phase3:production-release-adapter-input-builder-test";

const SWITCH_OFF_GATEWAY_PORT = Number(process.env.PHASE3_PROD_ADAPTER_INPUT_OFF_PORT || 3102);
const SWITCH_OFF_JWKS_PORT = Number(process.env.PHASE3_PROD_ADAPTER_INPUT_OFF_JWKS_PORT || 8112);

const SWITCH_ON_GATEWAY_PORT = Number(process.env.PHASE3_PROD_ADAPTER_INPUT_ON_PORT || 3103);
const SWITCH_ON_JWKS_PORT = Number(process.env.PHASE3_PROD_ADAPTER_INPUT_ON_JWKS_PORT || 8113);
...
async function main() {
  ...
  const summary = {
    ok: true,
    harness: "phase3.productionReleaseAdapterInputBuilder.v1",
