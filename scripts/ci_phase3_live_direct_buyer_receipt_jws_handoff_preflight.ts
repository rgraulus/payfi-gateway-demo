#!/usr/bin/env node
/**
 * scripts/ci_phase3_live_direct_buyer_receipt_jws_handoff_preflight.ts
 *
 * Milestone 3G / PR #228 harness.
 *
 ...
 * is reduced to sanitized handoff metadata bound to the fresh PAYMENT-REQUIRED
 * tuple.
 *
 * This is intentionally a guarded local preflight harness. It does not submit
 * the receipt JWS to Gateway, does not decode or verify the receipt in the
 * runtime path, does not release the protected resource, does not emit
 * PAYMENT-RESPONSE, does not call CRP fulfill, does not touch replay, and does
...
const GATEWAY_PORT = Number(process.env.PHASE3_LIVE_DIRECT_BUYER_RECEIPT_JWS_HANDOFF_PREFLIGHT_PORT || 3101);
const JWKS_PORT = Number(process.env.PHASE3_LIVE_DIRECT_BUYER_RECEIPT_JWS_HANDOFF_PREFLIGHT_JWKS_PORT || 8151);
const BASE = baseUrlForPort(GATEWAY_PORT);
const JWKS_BASE = baseUrlForPort(JWKS_PORT);
const LABEL = "phase3:live-direct-buyer-receipt-jws-handoff-preflight-test";
    ...
    JSON.stringify(
      {
        ...
        code: "usage",
        reason:
          "Usage: PHASE3_LIVE_DIRECT_BUYER_RECEIPT_JWS_HANDOFF_PREFLIGHT_HARNESS=true npm run phase3:live-direct-buyer-receipt-jws-handoff-preflight-test -- <local-wallet-proof.json>",
...
function buildSanitizedReceiptJwsHandoff(input: {
  ...
  return {
    contract: "phase3.liveDirectBuyer.receiptJwsRuntimeHandoffPreflight.v1",
    mode: "preflight_only",
    status: "handoff_ready",
    reason: "receipt_jws_handoff_metadata_built_without_runtime_submission",
    source: {
...
async function main() {
  ...
  if (String(process.env.PHASE3_LIVE_DIRECT_BUYER_RECEIPT_JWS_HANDOFF_PREFLIGHT_HARNESS ?? "").toLowerCase() !== "true") {
    console.error(
      JSON.stringify(
        {
          ok: false,
          ...
          reason:
            "Set PHASE3_LIVE_DIRECT_BUYER_RECEIPT_JWS_HANDOFF_PREFLIGHT_HARNESS=true to run this local-only harness.",
          rawProofPrinted: false,
          rawReceiptPrinted: false,
        },
        null,
  ...
  try {
    ...
    console.log(
      JSON.stringify(
        {
          ok: true,
          harness: "phase3.liveDirectBuyerReceiptJwsHandoffPreflight.v1",
