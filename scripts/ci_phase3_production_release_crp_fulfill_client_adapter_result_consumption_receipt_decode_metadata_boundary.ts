#!/usr/bin/env node
/**
 * PR #218 — Phase 3 production release CRP fulfill client adapter result consumption receipt decode activation preflight.
 *
 * This harness proves the Gateway exposes a receipt decode activation preflight after the
 * receipt decode enablement gate, while preserving non-execution and side-effect safety.
 *
 * - switch OFF + dry-run OFF => activation preflight inactive
 * - switch OFF + dry-run ON  => activation preflight inactive
 * - switch ON  + dry-run OFF => activation preflight inactive
 * - switch ON  + dry-run ON  + result consumption OFF => activation preflight inactive
 * - switch ON  + dry-run ON  + result consumption ON + receipt decode OFF => activation preflight inactive
 * - switch ON  + dry-run ON  + result consumption ON + receipt decode ON  => activation preflight ready, decode still unavailable
...
const LABEL = "phase3:production-release-crp-fulfill-client-adapter-result-consumption-receipt-decode-metadata-boundary-test";

const SWITCH_OFF_GATEWAY_PORT = Number(process.env.PHASE3_PROD_CRP_FULFILL_CLIENT_ADAPTER_RESULT_CONSUMPTION_RECEIPT_DECODE_METADATA_BOUNDARY_OFF_PORT || 3148);
const SWITCH_OFF_JWKS_PORT = Number(process.env.PHASE3_PROD_CRP_FULFILL_CLIENT_ADAPTER_RESULT_CONSUMPTION_RECEIPT_DECODE_METADATA_BOUNDARY_OFF_JWKS_PORT || 8158);

const SWITCH_ON_GATEWAY_PORT = Number(process.env.PHASE3_PROD_CRP_FULFILL_CLIENT_ADAPTER_RESULT_CONSUMPTION_RECEIPT_DECODE_METADATA_BOUNDARY_ON_PORT || 3149);
const SWITCH_ON_JWKS_PORT = Number(process.env.PHASE3_PROD_CRP_FULFILL_CLIENT_ADAPTER_RESULT_CONSUMPTION_RECEIPT_DECODE_METADATA_BOUNDARY_ON_JWKS_PORT || 8159);
...
async function withGatewayStack<T>(input: {
  ...
  console.log(
    `[${input.label}] PHASE3_GATEWAY_PRODUCTION_RELEASE_RECEIPT_DECODE_ENABLED=${input.receiptDecodeEnabled}`,
  );
  ...
  const previous = {
    ...
    PHASE3_GATEWAY_PRODUCTION_RELEASE_RECEIPT_DECODE_ENABLED:
      process.env.PHASE3_GATEWAY_PRODUCTION_RELEASE_RECEIPT_DECODE_ENABLED,
  ...
  process.env.PHASE3_GATEWAY_PRODUCTION_RELEASE_RESULT_CONSUMPTION_ENABLED = input.resultConsumptionEnabled
    ? "true"
    : "false";
  process.env.PHASE3_GATEWAY_PRODUCTION_RELEASE_RECEIPT_DECODE_ENABLED = input.receiptDecodeEnabled
    ? "true"
    : "false";
  ...
  return await withGatewayStack({
    ...
    resultConsumptionEnabled: input.resultConsumptionEnabled,
        ...
        productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeReadinessGateRequired:
          decision?.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeReadinessGateRequired === true,
        productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeReadinessGateObserved:
          decision?.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeReadinessGateObserved === true,
        productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeReadinessGateStatus:
          typeof decision?.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeReadinessGateStatus === "string"
            ? decision.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeReadinessGateStatus
            : null,
        productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeReadinessGateReason:
          typeof decision?.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeReadinessGateReason === "string"
            ? decision.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeReadinessGateReason
            : null,
        productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeReadinessGateBlockedBy:
          decision?.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeReadinessGateBlockedBy ?? null,
        productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeReadinessGate:
          decision?.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeReadinessGate ?? null,
        productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeReadinessGateReady:
          decision?.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeReadinessGateReady === true,
        productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeReadinessGateDryRunAuditObserved:
          decision?.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeReadinessGateDryRunAuditObserved === true,
        productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeReadinessGateDryRunAuditSanitized:
          decision?.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeReadinessGateDryRunAuditSanitized === true,
        productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeReadinessGateWouldAuditReceiptDecode:
          decision?.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeReadinessGateWouldAuditReceiptDecode === true,
        productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeReadinessGateWouldRequireReceiptJws:
          decision?.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeReadinessGateWouldRequireReceiptJws === true,
        productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeReadinessGateWouldRequireReceiptPayload:
          decision?.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeReadinessGateWouldRequireReceiptPayload === true,
        productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeReadinessGateWouldRequireReceiptDecode:
          decision?.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeReadinessGateWouldRequireReceiptDecode === true,
        productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeReadinessGateWouldRequireReceiptPayloadParse:
          decision?.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeReadinessGateWouldRequireReceiptPayloadParse === true,
        productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeReadinessGateWouldRequireFinalizedSettlement:
          decision?.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeReadinessGateWouldRequireFinalizedSettlement === true,
        productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeReadinessGateWouldRequireTupleBinding:
          decision?.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeReadinessGateWouldRequireTupleBinding === true,
        productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeReadinessGateWouldRequireNoReplay:
          decision?.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeReadinessGateWouldRequireNoReplay === true,
        productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeReadinessGateReceiptDecodeEnabled:
          decision?.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeReadinessGateReceiptDecodeEnabled === true,
        productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeReadinessGateReceiptPayloadParseEnabled:
          decision?.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeReadinessGateReceiptPayloadParseEnabled === true,
        productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeReadinessGateFinalizedSettlementVerificationEnabled:
          decision?.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeReadinessGateFinalizedSettlementVerificationEnabled === true,
        productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeReadinessGateTupleBindingVerificationEnabled:
          decision?.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeReadinessGateTupleBindingVerificationEnabled === true,
        productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeReadinessGateReplayCheckEnabled:
          decision?.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeReadinessGateReplayCheckEnabled === true,
        productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeReadinessGateProductionReleaseAuthorizationEnabled:
          decision?.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeReadinessGateProductionReleaseAuthorizationEnabled === true,
        productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeReadinessGateReceiptMaterialObserved:
          decision?.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeReadinessGateReceiptMaterialObserved === true,
        productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeReadinessGateReceiptMaterialHandled:
          decision?.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeReadinessGateReceiptMaterialHandled === true,
        productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeReadinessGateReceiptMaterialConsumed:
          decision?.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeReadinessGateReceiptMaterialConsumed === true,
        productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeReadinessGateReceiptDecoded:
          decision?.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeReadinessGateReceiptDecoded === true,
        productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeReadinessGateReceiptPayloadParsed:
          decision?.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeReadinessGateReceiptPayloadParsed === true,
        productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeReadinessGateAllowsReceiptDecode:
          decision?.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeReadinessGateAllowsReceiptDecode === true,
        productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeReadinessGateAllowsReceiptPayloadParse:
          decision?.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeReadinessGateAllowsReceiptPayloadParse === true,
        productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeReadinessGateAllowsFinalizedSettlementVerification:
          decision?.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeReadinessGateAllowsFinalizedSettlementVerification === true,
        productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeReadinessGateAllowsTupleBinding:
          decision?.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeReadinessGateAllowsTupleBinding === true,
        productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeReadinessGateAllowsReplayCheck:
          decision?.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeReadinessGateAllowsReplayCheck === true,
        productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeReadinessGateAllowsCrpFulfill:
          decision?.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeReadinessGateAllowsCrpFulfill === true,
        productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeReadinessGateAllowsProductionRelease:
          decision?.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeReadinessGateAllowsProductionRelease === true,
        productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeReadinessGateAdapterInvoked:
          decision?.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeReadinessGateAdapterInvoked === true,
        productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeReadinessGateExternalCallAttempted:
          decision?.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeReadinessGateExternalCallAttempted === true,
        productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeReadinessGateCrpCalled:
          decision?.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeReadinessGateCrpCalled === true,
        productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeReadinessGateCrpFulfillCalled:
          decision?.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeReadinessGateCrpFulfillCalled === true,
        productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeReadinessGateRawProofPrinted:
          decision?.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeReadinessGateRawProofPrinted === true,
        productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeReadinessGateRawReceiptPrinted:
          decision?.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeReadinessGateRawReceiptPrinted === true,
        productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeReadinessGateFinalizedSettlementVerified:
          decision?.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeReadinessGateFinalizedSettlementVerified === true,
        productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeReadinessGateTupleBindingVerified:
          decision?.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeReadinessGateTupleBindingVerified === true,
        productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeReadinessGateReplayTouched:
          decision?.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeReadinessGateReplayTouched === true,
        productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeReadinessGateSideEffectFree:
          decision?.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeReadinessGateSideEffectFree === true,
        productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeEnablementGateRequired:
          decision?.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeEnablementGateRequired === true,
        productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeEnablementGateObserved:
          decision?.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeEnablementGateObserved === true,
        productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeEnablementGateEnabled:
          decision?.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeEnablementGateEnabled === true,
        productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeEnablementGateReady:
          decision?.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeEnablementGateReady === true,
        productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeEnablementGateStatus:
          typeof decision?.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeEnablementGateStatus === "string"
            ? decision.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeEnablementGateStatus
            : null,
        productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeEnablementGateReason:
          typeof decision?.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeEnablementGateReason === "string"
            ? decision.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeEnablementGateReason
            : null,
        productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeEnablementGateBlockedBy:
          decision?.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeEnablementGateBlockedBy ?? null,
        productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeEnablementGate:
          decision?.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeEnablementGate ?? null,
        productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeEnablementGateAllowsReceiptDecode:
          decision?.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeEnablementGateAllowsReceiptDecode === true,
        productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeEnablementGateAllowsReceiptPayloadParse:
          decision?.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeEnablementGateAllowsReceiptPayloadParse === true,
        productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeEnablementGateAllowsFinalizedSettlementVerification:
          decision?.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeEnablementGateAllowsFinalizedSettlementVerification === true,
        productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeEnablementGateAllowsTupleBinding:
          decision?.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeEnablementGateAllowsTupleBinding === true,
        productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeEnablementGateAllowsReplayCheck:
          decision?.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeEnablementGateAllowsReplayCheck === true,
        productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeEnablementGateAllowsCrpFulfill:
          decision?.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeEnablementGateAllowsCrpFulfill === true,
        productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeEnablementGateAllowsProductionRelease:
          decision?.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeEnablementGateAllowsProductionRelease === true,
        productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeEnablementGateSideEffectFree:
          decision?.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeEnablementGateSideEffectFree === true,
        productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeActivationPreflightRequired:
          decision?.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeActivationPreflightRequired === true,
        productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeActivationPreflightObserved:
          decision?.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeActivationPreflightObserved === true,
        productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeActivationPreflightReady:
          decision?.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeActivationPreflightReady === true,
        productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeActivationPreflightStatus:
          typeof decision?.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeActivationPreflightStatus === "string"
            ? decision.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeActivationPreflightStatus
            : null,
        productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeActivationPreflightReason:
          typeof decision?.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeActivationPreflightReason === "string"
            ? decision.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeActivationPreflightReason
            : null,
        productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeActivationPreflightBlockedBy:
          decision?.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeActivationPreflightBlockedBy ?? null,
        productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeActivationPreflight:
          decision?.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeActivationPreflight ?? null,
        productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeActivationPreflightAllowsReceiptDecode:
          decision?.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeActivationPreflightAllowsReceiptDecode === true,
        productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeActivationPreflightAllowsReceiptPayloadParse:
          decision?.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeActivationPreflightAllowsReceiptPayloadParse === true,
        productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeActivationPreflightAllowsFinalizedSettlementVerification:
          decision?.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeActivationPreflightAllowsFinalizedSettlementVerification === true,
        productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeActivationPreflightAllowsTupleBinding:
          decision?.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeActivationPreflightAllowsTupleBinding === true,
        productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeActivationPreflightAllowsReplayCheck:
          decision?.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeActivationPreflightAllowsReplayCheck === true,
        productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeActivationPreflightAllowsCrpFulfill:
          decision?.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeActivationPreflightAllowsCrpFulfill === true,
        productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeActivationPreflightAllowsProductionRelease:
          decision?.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeActivationPreflightAllowsProductionRelease === true,
        productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeActivationPreflightReceiptMaterialObserved:
          decision?.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeActivationPreflightReceiptMaterialObserved === true,
        productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeActivationPreflightReceiptMaterialHandled:
          decision?.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeActivationPreflightReceiptMaterialHandled === true,
        productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeActivationPreflightReceiptMaterialConsumed:
          decision?.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeActivationPreflightReceiptMaterialConsumed === true,
        productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeActivationPreflightReceiptDecoded:
          decision?.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeActivationPreflightReceiptDecoded === true,
        productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeActivationPreflightReceiptPayloadParsed:
          decision?.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeActivationPreflightReceiptPayloadParsed === true,
        productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeActivationPreflightFinalizedSettlementVerified:
          decision?.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeActivationPreflightFinalizedSettlementVerified === true,
        productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeActivationPreflightTupleBindingVerified:
          decision?.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeActivationPreflightTupleBindingVerified === true,
        productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeActivationPreflightReplayTouched:
          decision?.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeActivationPreflightReplayTouched === true,
        productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeActivationPreflightAdapterInvoked:
          decision?.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeActivationPreflightAdapterInvoked === true,
        productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeActivationPreflightExternalCallAttempted:
          decision?.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeActivationPreflightExternalCallAttempted === true,
        productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeActivationPreflightCrpCalled:
          decision?.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeActivationPreflightCrpCalled === true,
        productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeActivationPreflightCrpFulfillCalled:
          decision?.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeActivationPreflightCrpFulfillCalled === true,
        productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeActivationPreflightRawProofPrinted:
          decision?.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeActivationPreflightRawProofPrinted === true,
        productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeActivationPreflightRawReceiptPrinted:
          decision?.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeActivationPreflightRawReceiptPrinted === true,
        productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeActivationPreflightSideEffectFree:
          decision?.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeActivationPreflightSideEffectFree === true,
        productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeNoopDecoderRequired:
          decision?.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeNoopDecoderRequired === true,
        productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeNoopDecoderObserved:
          decision?.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeNoopDecoderObserved === true,
        productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeNoopDecoderReady:
          decision?.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeNoopDecoderReady === true,
        productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeNoopDecoderStatus:
          typeof decision?.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeNoopDecoderStatus === "string"
            ? decision.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeNoopDecoderStatus
            : null,
        productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeNoopDecoderReason:
          typeof decision?.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeNoopDecoderReason === "string"
            ? decision.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeNoopDecoderReason
            : null,
        productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeNoopDecoderBlockedBy:
          decision?.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeNoopDecoderBlockedBy ?? null,
        productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeNoopDecoder:
          decision?.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeNoopDecoder ?? null,
        productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeNoopDecoderAllowsReceiptDecode:
          decision?.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeNoopDecoderAllowsReceiptDecode === true,
        productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeNoopDecoderAllowsReceiptPayloadParse:
          decision?.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeNoopDecoderAllowsReceiptPayloadParse === true,
        productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeNoopDecoderAllowsFinalizedSettlementVerification:
          decision?.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeNoopDecoderAllowsFinalizedSettlementVerification === true,
        productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeNoopDecoderAllowsTupleBinding:
          decision?.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeNoopDecoderAllowsTupleBinding === true,
        productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeNoopDecoderAllowsReplayCheck:
          decision?.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeNoopDecoderAllowsReplayCheck === true,
        productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeNoopDecoderAllowsCrpFulfill:
          decision?.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeNoopDecoderAllowsCrpFulfill === true,
        productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeNoopDecoderAllowsProductionRelease:
          decision?.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeNoopDecoderAllowsProductionRelease === true,
        productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeNoopDecoderReceiptMaterialObserved:
          decision?.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeNoopDecoderReceiptMaterialObserved === true,
        productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeNoopDecoderReceiptMaterialHandled:
          decision?.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeNoopDecoderReceiptMaterialHandled === true,
        productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeNoopDecoderReceiptMaterialConsumed:
          decision?.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeNoopDecoderReceiptMaterialConsumed === true,
        productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeNoopDecoderReceiptDecoded:
          decision?.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeNoopDecoderReceiptDecoded === true,
        productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeNoopDecoderReceiptPayloadParsed:
          decision?.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeNoopDecoderReceiptPayloadParsed === true,
        productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeNoopDecoderFinalizedSettlementVerified:
          decision?.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeNoopDecoderFinalizedSettlementVerified === true,
        productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeNoopDecoderTupleBindingVerified:
          decision?.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeNoopDecoderTupleBindingVerified === true,
        productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeNoopDecoderReplayTouched:
          decision?.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeNoopDecoderReplayTouched === true,
        productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeNoopDecoderAdapterInvoked:
          decision?.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeNoopDecoderAdapterInvoked === true,
        productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeNoopDecoderExternalCallAttempted:
          decision?.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeNoopDecoderExternalCallAttempted === true,
        productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeNoopDecoderCrpCalled:
          decision?.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeNoopDecoderCrpCalled === true,
        productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeNoopDecoderCrpFulfillCalled:
          decision?.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeNoopDecoderCrpFulfillCalled === true,
        productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeNoopDecoderRawProofPrinted:
          decision?.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeNoopDecoderRawProofPrinted === true,
        productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeNoopDecoderRawReceiptPrinted:
          decision?.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeNoopDecoderRawReceiptPrinted === true,
        productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeNoopDecoderSideEffectFree:
          decision?.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeNoopDecoderSideEffectFree === true,
        productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeMetadataBoundaryRequired:
          decision?.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeMetadataBoundaryRequired === true,
        productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeMetadataBoundaryObserved:
          decision?.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeMetadataBoundaryObserved === true,
        productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeMetadataBoundaryReady:
          decision?.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeMetadataBoundaryReady === true,
        productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeMetadataBoundaryStatus:
          typeof decision?.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeMetadataBoundaryStatus === "string"
            ? decision.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeMetadataBoundaryStatus
            : null,
        productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeMetadataBoundaryReason:
          typeof decision?.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeMetadataBoundaryReason === "string"
            ? decision.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeMetadataBoundaryReason
            : null,
        productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeMetadataBoundaryBlockedBy:
          decision?.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeMetadataBoundaryBlockedBy ?? null,
        productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeMetadataBoundary:
          decision?.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeMetadataBoundary ?? null,
        productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeMetadataBoundaryAllowsReceiptMaterialObservation:
          decision?.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeMetadataBoundaryAllowsReceiptMaterialObservation === true,
        productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeMetadataBoundaryAllowsReceiptMaterialHandling:
          decision?.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeMetadataBoundaryAllowsReceiptMaterialHandling === true,
        productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeMetadataBoundaryAllowsReceiptMaterialConsumption:
          decision?.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeMetadataBoundaryAllowsReceiptMaterialConsumption === true,
        productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeMetadataBoundaryAllowsReceiptDecode:
          decision?.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeMetadataBoundaryAllowsReceiptDecode === true,
        productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeMetadataBoundaryAllowsReceiptPayloadParse:
          decision?.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeMetadataBoundaryAllowsReceiptPayloadParse === true,
        productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeMetadataBoundaryAllowsFinalizedSettlementVerification:
          decision?.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeMetadataBoundaryAllowsFinalizedSettlementVerification === true,
        productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeMetadataBoundaryAllowsTupleBinding:
          decision?.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeMetadataBoundaryAllowsTupleBinding === true,
        productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeMetadataBoundaryAllowsReplayCheck:
          decision?.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeMetadataBoundaryAllowsReplayCheck === true,
        productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeMetadataBoundaryAllowsCrpFulfill:
          decision?.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeMetadataBoundaryAllowsCrpFulfill === true,
        productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeMetadataBoundaryAllowsProductionRelease:
          decision?.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeMetadataBoundaryAllowsProductionRelease === true,
        productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeMetadataBoundaryReceiptMaterialObserved:
          decision?.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeMetadataBoundaryReceiptMaterialObserved === true,
        productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeMetadataBoundaryReceiptMaterialHandled:
          decision?.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeMetadataBoundaryReceiptMaterialHandled === true,
        productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeMetadataBoundaryReceiptMaterialConsumed:
          decision?.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeMetadataBoundaryReceiptMaterialConsumed === true,
        productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeMetadataBoundaryReceiptDecoded:
          decision?.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeMetadataBoundaryReceiptDecoded === true,
        productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeMetadataBoundaryReceiptPayloadParsed:
          decision?.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeMetadataBoundaryReceiptPayloadParsed === true,
        productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeMetadataBoundaryFinalizedSettlementVerified:
          decision?.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeMetadataBoundaryFinalizedSettlementVerified === true,
        productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeMetadataBoundaryTupleBindingVerified:
          decision?.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeMetadataBoundaryTupleBindingVerified === true,
        productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeMetadataBoundaryReplayTouched:
          decision?.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeMetadataBoundaryReplayTouched === true,
        productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeMetadataBoundaryAdapterInvoked:
          decision?.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeMetadataBoundaryAdapterInvoked === true,
        productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeMetadataBoundaryExternalCallAttempted:
          decision?.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeMetadataBoundaryExternalCallAttempted === true,
        productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeMetadataBoundaryCrpCalled:
          decision?.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeMetadataBoundaryCrpCalled === true,
        productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeMetadataBoundaryCrpFulfillCalled:
          decision?.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeMetadataBoundaryCrpFulfillCalled === true,
        productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeMetadataBoundaryRawProofPrinted:
          decision?.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeMetadataBoundaryRawProofPrinted === true,
        productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeMetadataBoundaryRawReceiptPrinted:
          decision?.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeMetadataBoundaryRawReceiptPrinted === true,
        productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeMetadataBoundarySideEffectFree:
          decision?.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeMetadataBoundarySideEffectFree === true,
...
async function main() {
  ...
  const switchOnDryRunOnResultConsumptionOnReceiptDecodeOn = await runScenario({
    label: `${LABEL}:switch-on-dry-run-on-result-consumption-on-receipt-decode-on`,
    gatewayPort: SWITCH_ON_GATEWAY_PORT,
    jwksPort: SWITCH_ON_JWKS_PORT,
    productionSwitchEnabled: true,
    dryRunEnabled: true,
    resultConsumptionEnabled: true,
    receiptDecodeEnabled: true,
  });

  const summary = {
    ok: true,
    harness: "phase3.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeMetadataBoundary.v1",
    ...
    crpFulfillClientAdapterResultConsumptionReceiptMaterialHandlingGateSideEffectFree:
      ...
      switchOnDryRunOnResultConsumptionOn.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptMaterialHandlingGateCrpFulfillCalled ===
        false,
    crpFulfillClientAdapterResultConsumptionReceiptDecodeReadinessGateInactiveUntilReceiptDecodeDryRunAuditObserved:
      switchOffDryRunOff.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeReadinessGateRequired ===
        false &&
      switchOffDryRunOn.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeReadinessGateRequired ===
        false &&
      switchOnDryRunOff.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeReadinessGateRequired ===
        false &&
      switchOnDryRunOn.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeReadinessGateRequired ===
        false,

    crpFulfillClientAdapterResultConsumptionReceiptDecodeReadinessGateObservedAfterReceiptDecodeDryRunAudit:
      switchOnDryRunOnResultConsumptionOn.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeReadinessGateRequired ===
        true &&
      switchOnDryRunOnResultConsumptionOn.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeReadinessGateObserved ===
        true &&
      switchOnDryRunOnResultConsumptionOn.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeReadinessGateStatus ===
        "blocked" &&
      switchOnDryRunOnResultConsumptionOn.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeReadinessGateReason ===
        "production_release_crp_fulfill_client_adapter_result_consumption_receipt_decode_readiness_gate_blocked" &&
      switchOnDryRunOnResultConsumptionOn.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeReadinessGateBlockedBy ===
        "production_release_crp_fulfill_client_adapter_result_consumption_receipt_decode_not_enabled",

      crpFulfillClientAdapterResultConsumptionReceiptDecodeReadinessGatePreservesDryRunAuditContext:
        switchOnDryRunOnResultConsumptionOn.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeReadinessGate
          ?.source?.dryRunAuditStatus === "recorded" &&
        switchOnDryRunOnResultConsumptionOn.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeReadinessGate
          ?.source?.dryRunAuditReason ===
        ...
        switchOnDryRunOnResultConsumptionOn.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeReadinessGateDryRunAuditObserved ===
          true &&
        switchOnDryRunOnResultConsumptionOn.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeReadinessGateDryRunAuditSanitized ===
          true,

      crpFulfillClientAdapterResultConsumptionReceiptDecodeReadinessGateRecordsReadinessChecklist:
        switchOnDryRunOnResultConsumptionOn.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeReadinessGateWouldAuditReceiptDecode ===
          true &&
        switchOnDryRunOnResultConsumptionOn.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeReadinessGateWouldRequireReceiptJws ===
          true &&
        switchOnDryRunOnResultConsumptionOn.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeReadinessGateWouldRequireReceiptPayload ===
          true &&
        switchOnDryRunOnResultConsumptionOn.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeReadinessGateWouldRequireReceiptDecode ===
          true &&
        switchOnDryRunOnResultConsumptionOn.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeReadinessGateWouldRequireReceiptPayloadParse ===
          true &&
        switchOnDryRunOnResultConsumptionOn.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeReadinessGateWouldRequireFinalizedSettlement ===
          true &&
        switchOnDryRunOnResultConsumptionOn.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeReadinessGateWouldRequireTupleBinding ===
          true &&
        switchOnDryRunOnResultConsumptionOn.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeReadinessGateWouldRequireNoReplay ===
          true &&
        switchOnDryRunOnResultConsumptionOn.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeReadinessGate
          ?.readiness?.wouldAuditReceiptDecode === true &&
        switchOnDryRunOnResultConsumptionOn.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeReadinessGate
          ?.readiness?.wouldRequireNoReplay === true,

      crpFulfillClientAdapterResultConsumptionReceiptDecodeReadinessGateBlocksReceiptDecode:
        switchOnDryRunOnResultConsumptionOn.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeReadinessGateReceiptDecodeEnabled ===
          false &&
        switchOnDryRunOnResultConsumptionOn.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeReadinessGateReceiptPayloadParseEnabled ===
          false &&
        switchOnDryRunOnResultConsumptionOn.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeReadinessGateFinalizedSettlementVerificationEnabled ===
          false &&
        switchOnDryRunOnResultConsumptionOn.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeReadinessGateTupleBindingVerificationEnabled ===
          false &&
        switchOnDryRunOnResultConsumptionOn.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeReadinessGateReplayCheckEnabled ===
          false &&
        switchOnDryRunOnResultConsumptionOn.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeReadinessGateProductionReleaseAuthorizationEnabled ===
          false &&
        switchOnDryRunOnResultConsumptionOn.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeReadinessGate
          ?.readiness?.receiptDecodeEnabled === false &&
        switchOnDryRunOnResultConsumptionOn.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeReadinessGate
          ?.readiness?.receiptPayloadParseEnabled === false &&
        switchOnDryRunOnResultConsumptionOn.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeReadinessGate
          ?.readiness?.finalizedSettlementVerificationEnabled === false &&
        switchOnDryRunOnResultConsumptionOn.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeReadinessGate
          ?.readiness?.tupleBindingVerificationEnabled === false &&
        switchOnDryRunOnResultConsumptionOn.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeReadinessGate
          ?.readiness?.replayCheckEnabled === false &&
        switchOnDryRunOnResultConsumptionOn.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeReadinessGate
          ?.readiness?.productionReleaseAuthorizationEnabled === false,

      crpFulfillClientAdapterResultConsumptionReceiptDecodeReadinessGateDoesNotObserveReceiptMaterial:
        switchOnDryRunOnResultConsumptionOn.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeReadinessGateReceiptMaterialObserved ===
          false &&
        switchOnDryRunOnResultConsumptionOn.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeReadinessGate
          ?.safety?.receiptMaterialObserved === false,

      crpFulfillClientAdapterResultConsumptionReceiptDecodeReadinessGateDoesNotHandleOrConsumeReceiptMaterial:
        switchOnDryRunOnResultConsumptionOn.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeReadinessGateReceiptMaterialHandled ===
          false &&
        switchOnDryRunOnResultConsumptionOn.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeReadinessGateReceiptMaterialConsumed ===
          false &&
        switchOnDryRunOnResultConsumptionOn.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeReadinessGate
          ?.safety?.receiptMaterialHandled === false &&
        switchOnDryRunOnResultConsumptionOn.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeReadinessGate
          ?.safety?.receiptMaterialConsumed === false,

      crpFulfillClientAdapterResultConsumptionReceiptDecodeReadinessGateDoesNotDecodeOrParseReceipt:
        switchOnDryRunOnResultConsumptionOn.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeReadinessGateReceiptDecoded ===
          false &&
        switchOnDryRunOnResultConsumptionOn.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeReadinessGateReceiptPayloadParsed ===
          false &&
        switchOnDryRunOnResultConsumptionOn.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeReadinessGateAllowsReceiptDecode ===
          false &&
        switchOnDryRunOnResultConsumptionOn.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeReadinessGateAllowsReceiptPayloadParse ===
          false,

    crpFulfillClientAdapterResultConsumptionReceiptDecodeReadinessGateDoesNotVerifySettlementOrTuple:
      switchOnDryRunOnResultConsumptionOn.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeReadinessGateFinalizedSettlementVerified ===
        false &&
      switchOnDryRunOnResultConsumptionOn.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeReadinessGateTupleBindingVerified ===
        false &&
      switchOnDryRunOnResultConsumptionOn.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeReadinessGateAllowsFinalizedSettlementVerification ===
        false &&
      switchOnDryRunOnResultConsumptionOn.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeReadinessGateAllowsTupleBinding ===
        false,

    crpFulfillClientAdapterResultConsumptionReceiptDecodeReadinessGateDoesNotTouchReplay:
      switchOnDryRunOnResultConsumptionOn.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeReadinessGateReplayTouched ===
        false &&
      switchOnDryRunOnResultConsumptionOn.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeReadinessGateAllowsReplayCheck ===
        false,

    crpFulfillClientAdapterResultConsumptionReceiptDecodeReadinessGateDoesNotAuthorizeProductionRelease:
      switchOnDryRunOnResultConsumptionOn.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeReadinessGateAllowsProductionRelease ===
        false &&
      switchOnDryRunOnResultConsumptionOn.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeReadinessGate
        ?.safety?.productionReleaseAuthorized === false &&
      switchOnDryRunOnResultConsumptionOn.productionRelease === false,

    crpFulfillClientAdapterResultConsumptionReceiptDecodeReadinessGateSanitized:
      switchOnDryRunOnResultConsumptionOn.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeReadinessGate
        ?.safety?.sanitized === true &&
      switchOnDryRunOnResultConsumptionOn.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeReadinessGateRawProofPrinted ===
        false &&
      switchOnDryRunOnResultConsumptionOn.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeReadinessGateRawReceiptPrinted ===
        false,

    crpFulfillClientAdapterResultConsumptionReceiptDecodeReadinessGateSideEffectFree:
      switchOnDryRunOnResultConsumptionOn.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeReadinessGateSideEffectFree ===
        true &&
      switchOnDryRunOnResultConsumptionOn.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeReadinessGateAdapterInvoked ===
        false &&
      switchOnDryRunOnResultConsumptionOn.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeReadinessGateExternalCallAttempted ===
        false &&
      switchOnDryRunOnResultConsumptionOn.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeReadinessGateCrpCalled ===
        false &&
      switchOnDryRunOnResultConsumptionOn.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeReadinessGateCrpFulfillCalled ===
        false,
    crpFulfillClientAdapterResultConsumptionReceiptDecodeEnablementGateInactiveUntilReadinessGateObserved:
      switchOffDryRunOff.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeEnablementGateRequired === false &&
      switchOffDryRunOn.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeEnablementGateRequired === false &&
      switchOnDryRunOff.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeEnablementGateRequired === false &&
      switchOnDryRunOn.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeEnablementGateRequired === false,

    crpFulfillClientAdapterResultConsumptionReceiptDecodeEnablementGateBlockedWhenFlagOff:
      switchOnDryRunOnResultConsumptionOn.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeEnablementGateRequired === true &&
      switchOnDryRunOnResultConsumptionOn.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeEnablementGateObserved === true &&
      switchOnDryRunOnResultConsumptionOn.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeEnablementGateEnabled === false &&
      switchOnDryRunOnResultConsumptionOn.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeEnablementGateReady === false &&
      switchOnDryRunOnResultConsumptionOn.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeEnablementGateStatus === "blocked" &&
      switchOnDryRunOnResultConsumptionOn.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeEnablementGateReason === "production_release_crp_fulfill_client_adapter_result_consumption_receipt_decode_enablement_disabled" &&
      switchOnDryRunOnResultConsumptionOn.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeEnablementGateBlockedBy === "production_release_crp_fulfill_client_adapter_result_consumption_receipt_decode_enablement_disabled",

    crpFulfillClientAdapterResultConsumptionReceiptDecodeEnablementGateEnabledWhenFlagOnButNotReady:
      switchOnDryRunOnResultConsumptionOnReceiptDecodeOn.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeEnablementGateRequired === true &&
      switchOnDryRunOnResultConsumptionOnReceiptDecodeOn.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeEnablementGateObserved === true &&
      switchOnDryRunOnResultConsumptionOnReceiptDecodeOn.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeEnablementGateEnabled === true &&
      switchOnDryRunOnResultConsumptionOnReceiptDecodeOn.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeEnablementGateReady === false &&
      switchOnDryRunOnResultConsumptionOnReceiptDecodeOn.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeEnablementGateStatus === "enabled" &&
      switchOnDryRunOnResultConsumptionOnReceiptDecodeOn.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeEnablementGateReason === "production_release_crp_fulfill_client_adapter_result_consumption_receipt_decode_enablement_enabled" &&
      switchOnDryRunOnResultConsumptionOnReceiptDecodeOn.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeEnablementGateBlockedBy === null &&
      switchOnDryRunOnResultConsumptionOnReceiptDecodeOn.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeEnablementGate?.contract === "phase3.productionRelease.crpFulfillClientAdapter.resultConsumptionReceiptDecodeEnablementGate.v1" &&
      switchOnDryRunOnResultConsumptionOnReceiptDecodeOn.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeEnablementGate?.enablement?.flag === "PHASE3_GATEWAY_PRODUCTION_RELEASE_RECEIPT_DECODE_ENABLED" &&
      switchOnDryRunOnResultConsumptionOnReceiptDecodeOn.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeEnablementGate?.enablement?.enabled === true,

    crpFulfillClientAdapterResultConsumptionReceiptDecodeEnablementGateStillBlocksAllExecution:
      switchOnDryRunOnResultConsumptionOnReceiptDecodeOn.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeEnablementGateAllowsReceiptDecode === false &&
      switchOnDryRunOnResultConsumptionOnReceiptDecodeOn.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeEnablementGateAllowsReceiptPayloadParse === false &&
      switchOnDryRunOnResultConsumptionOnReceiptDecodeOn.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeEnablementGateAllowsFinalizedSettlementVerification === false &&
      switchOnDryRunOnResultConsumptionOnReceiptDecodeOn.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeEnablementGateAllowsTupleBinding === false &&
      switchOnDryRunOnResultConsumptionOnReceiptDecodeOn.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeEnablementGateAllowsReplayCheck === false &&
      switchOnDryRunOnResultConsumptionOnReceiptDecodeOn.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeEnablementGateAllowsCrpFulfill === false &&
      switchOnDryRunOnResultConsumptionOnReceiptDecodeOn.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeEnablementGateAllowsProductionRelease === false &&
      switchOnDryRunOnResultConsumptionOnReceiptDecodeOn.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeEnablementGateSideEffectFree === true &&
      switchOnDryRunOnResultConsumptionOnReceiptDecodeOn.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeEnablementGate?.safety?.sanitized === true &&
      switchOnDryRunOnResultConsumptionOnReceiptDecodeOn.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeEnablementGate?.safety?.receiptMaterialObserved === false &&
      switchOnDryRunOnResultConsumptionOnReceiptDecodeOn.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeEnablementGate?.safety?.receiptMaterialHandled === false &&
      switchOnDryRunOnResultConsumptionOnReceiptDecodeOn.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeEnablementGate?.safety?.receiptMaterialConsumed === false &&
      switchOnDryRunOnResultConsumptionOnReceiptDecodeOn.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeEnablementGate?.safety?.receiptDecoded === false &&
      switchOnDryRunOnResultConsumptionOnReceiptDecodeOn.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeEnablementGate?.safety?.receiptPayloadParsed === false &&
      switchOnDryRunOnResultConsumptionOnReceiptDecodeOn.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeEnablementGate?.safety?.finalizedSettlementVerified === false &&
      switchOnDryRunOnResultConsumptionOnReceiptDecodeOn.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeEnablementGate?.safety?.tupleBindingVerified === false &&
      switchOnDryRunOnResultConsumptionOnReceiptDecodeOn.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeEnablementGate?.safety?.replayTouched === false &&
      switchOnDryRunOnResultConsumptionOnReceiptDecodeOn.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeEnablementGate?.safety?.adapterInvoked === false &&
      switchOnDryRunOnResultConsumptionOnReceiptDecodeOn.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeEnablementGate?.safety?.externalCallAttempted === false &&
      switchOnDryRunOnResultConsumptionOnReceiptDecodeOn.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeEnablementGate?.safety?.crpCalled === false &&
      switchOnDryRunOnResultConsumptionOnReceiptDecodeOn.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeEnablementGate?.safety?.crpFulfillCalled === false &&
      switchOnDryRunOnResultConsumptionOnReceiptDecodeOn.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeEnablementGate?.safety?.rawProofPrinted === false &&
      switchOnDryRunOnResultConsumptionOnReceiptDecodeOn.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeEnablementGate?.safety?.rawReceiptPrinted === false &&
      switchOnDryRunOnResultConsumptionOnReceiptDecodeOn.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeEnablementGate?.safety?.productionReleaseAuthorized === false &&
      switchOnDryRunOnResultConsumptionOnReceiptDecodeOn.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeEnablementGate?.safety?.productionRelease === false &&
      switchOnDryRunOnResultConsumptionOnReceiptDecodeOn.productionRelease === false,

    crpFulfillClientAdapterResultConsumptionReceiptDecodeActivationPreflightInactiveUntilEnablementGateEnabled:
      switchOffDryRunOff.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeActivationPreflightRequired === false &&
      switchOffDryRunOff.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeActivationPreflightObserved === false &&
      switchOffDryRunOff.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeActivationPreflightStatus === "inactive" &&
      switchOffDryRunOff.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeActivationPreflight === null &&
      switchOffDryRunOn.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeActivationPreflightRequired === false &&
      switchOffDryRunOn.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeActivationPreflightObserved === false &&
      switchOffDryRunOn.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeActivationPreflightStatus === "inactive" &&
      switchOffDryRunOn.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeActivationPreflight === null &&
      switchOnDryRunOff.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeActivationPreflightRequired === false &&
      switchOnDryRunOff.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeActivationPreflightObserved === false &&
      switchOnDryRunOff.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeActivationPreflightStatus === "inactive" &&
      switchOnDryRunOff.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeActivationPreflight === null &&
      switchOnDryRunOn.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeActivationPreflightRequired === false &&
      switchOnDryRunOn.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeActivationPreflightObserved === false &&
      switchOnDryRunOn.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeActivationPreflightStatus === "inactive" &&
      switchOnDryRunOn.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeActivationPreflight === null &&
      switchOnDryRunOnResultConsumptionOn.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeEnablementGateEnabled === false &&
      switchOnDryRunOnResultConsumptionOn.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeActivationPreflightRequired === false &&
      switchOnDryRunOnResultConsumptionOn.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeActivationPreflightObserved === false &&
      switchOnDryRunOnResultConsumptionOn.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeActivationPreflightStatus === "inactive" &&
      switchOnDryRunOnResultConsumptionOn.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeActivationPreflight === null,

    crpFulfillClientAdapterResultConsumptionReceiptDecodeActivationPreflightObservedAfterEnablementGateEnabled:
      switchOnDryRunOnResultConsumptionOnReceiptDecodeOn.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeEnablementGateEnabled === true &&
      switchOnDryRunOnResultConsumptionOnReceiptDecodeOn.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeActivationPreflightRequired === true &&
      switchOnDryRunOnResultConsumptionOnReceiptDecodeOn.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeActivationPreflightObserved === true &&
      switchOnDryRunOnResultConsumptionOnReceiptDecodeOn.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeActivationPreflightReady === false &&
      switchOnDryRunOnResultConsumptionOnReceiptDecodeOn.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeActivationPreflightStatus === "preflight_ready" &&
      switchOnDryRunOnResultConsumptionOnReceiptDecodeOn.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeActivationPreflightReason === "production_release_crp_fulfill_client_adapter_result_consumption_receipt_decode_activation_preflight_ready" &&
      switchOnDryRunOnResultConsumptionOnReceiptDecodeOn.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeActivationPreflightBlockedBy === null &&
      switchOnDryRunOnResultConsumptionOnReceiptDecodeOn.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeActivationPreflight?.contract === "phase3.productionRelease.crpFulfillClientAdapter.resultConsumptionReceiptDecodeActivationPreflight.v1" &&
      switchOnDryRunOnResultConsumptionOnReceiptDecodeOn.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeActivationPreflight?.mode === "contract_only",

    crpFulfillClientAdapterResultConsumptionReceiptDecodeActivationPreflightPreservesEnablementContextAndRecordsRequirements:
      switchOnDryRunOnResultConsumptionOnReceiptDecodeOn.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeActivationPreflight
        ?.source?.enablementGateStatus ===
        switchOnDryRunOnResultConsumptionOnReceiptDecodeOn.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeEnablementGateStatus &&
      switchOnDryRunOnResultConsumptionOnReceiptDecodeOn.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeActivationPreflight
        ?.source?.enablementGateReason ===
        switchOnDryRunOnResultConsumptionOnReceiptDecodeOn.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeEnablementGateReason &&
      switchOnDryRunOnResultConsumptionOnReceiptDecodeOn.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeActivationPreflight
        ?.source?.enablementGateEnabled === true &&
      switchOnDryRunOnResultConsumptionOnReceiptDecodeOn.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeActivationPreflight
        ?.source?.enablementGateContract ===
      ...
      switchOnDryRunOnResultConsumptionOnReceiptDecodeOn.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeActivationPreflight
        ?.preflight?.receiptPayloadRequired === true &&
      switchOnDryRunOnResultConsumptionOnReceiptDecodeOn.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeActivationPreflight
        ?.preflight?.decoderImplementationRequired === true &&
      switchOnDryRunOnResultConsumptionOnReceiptDecodeOn.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeActivationPreflight
        ?.preflight?.decoderImplementationAvailable === false &&
      switchOnDryRunOnResultConsumptionOnReceiptDecodeOn.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeActivationPreflight
        ?.preflight?.receiptMaterialObservationAllowed === false &&
      switchOnDryRunOnResultConsumptionOnReceiptDecodeOn.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeActivationPreflight
        ?.preflight?.receiptDecodeAllowed === false &&
      switchOnDryRunOnResultConsumptionOnReceiptDecodeOn.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeActivationPreflight
        ?.preflight?.receiptPayloadParseAllowed === false &&
      switchOnDryRunOnResultConsumptionOnReceiptDecodeOn.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeActivationPreflight
        ?.preflight?.finalizedSettlementVerificationRemainsDownstream === true &&
      switchOnDryRunOnResultConsumptionOnReceiptDecodeOn.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeActivationPreflight
        ?.preflight?.tupleBindingRemainsDownstream === true &&
      switchOnDryRunOnResultConsumptionOnReceiptDecodeOn.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeActivationPreflight
        ?.preflight?.replayCheckRemainsDownstream === true,

    crpFulfillClientAdapterResultConsumptionReceiptDecodeActivationPreflightStillBlocksAllExecution:
      switchOnDryRunOnResultConsumptionOnReceiptDecodeOn.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeActivationPreflightAllowsReceiptDecode === false &&
      switchOnDryRunOnResultConsumptionOnReceiptDecodeOn.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeActivationPreflightAllowsReceiptPayloadParse === false &&
      switchOnDryRunOnResultConsumptionOnReceiptDecodeOn.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeActivationPreflightAllowsFinalizedSettlementVerification === false &&
      switchOnDryRunOnResultConsumptionOnReceiptDecodeOn.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeActivationPreflightAllowsTupleBinding === false &&
      switchOnDryRunOnResultConsumptionOnReceiptDecodeOn.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeActivationPreflightAllowsReplayCheck === false &&
      switchOnDryRunOnResultConsumptionOnReceiptDecodeOn.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeActivationPreflightAllowsCrpFulfill === false &&
      switchOnDryRunOnResultConsumptionOnReceiptDecodeOn.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeActivationPreflightAllowsProductionRelease === false &&
      switchOnDryRunOnResultConsumptionOnReceiptDecodeOn.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeActivationPreflightReceiptMaterialObserved === false &&
      switchOnDryRunOnResultConsumptionOnReceiptDecodeOn.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeActivationPreflightReceiptMaterialHandled === false &&
      switchOnDryRunOnResultConsumptionOnReceiptDecodeOn.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeActivationPreflightReceiptMaterialConsumed === false &&
      switchOnDryRunOnResultConsumptionOnReceiptDecodeOn.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeActivationPreflightReceiptDecoded === false &&
      switchOnDryRunOnResultConsumptionOnReceiptDecodeOn.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeActivationPreflightReceiptPayloadParsed === false &&
      switchOnDryRunOnResultConsumptionOnReceiptDecodeOn.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeActivationPreflightFinalizedSettlementVerified === false &&
      switchOnDryRunOnResultConsumptionOnReceiptDecodeOn.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeActivationPreflightTupleBindingVerified === false &&
      switchOnDryRunOnResultConsumptionOnReceiptDecodeOn.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeActivationPreflightReplayTouched === false &&
      switchOnDryRunOnResultConsumptionOnReceiptDecodeOn.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeActivationPreflightAdapterInvoked === false &&
      switchOnDryRunOnResultConsumptionOnReceiptDecodeOn.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeActivationPreflightExternalCallAttempted === false &&
      switchOnDryRunOnResultConsumptionOnReceiptDecodeOn.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeActivationPreflightCrpCalled === false &&
      switchOnDryRunOnResultConsumptionOnReceiptDecodeOn.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeActivationPreflightCrpFulfillCalled === false &&
      switchOnDryRunOnResultConsumptionOnReceiptDecodeOn.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeActivationPreflightRawProofPrinted === false &&
      switchOnDryRunOnResultConsumptionOnReceiptDecodeOn.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeActivationPreflightRawReceiptPrinted === false &&
      switchOnDryRunOnResultConsumptionOnReceiptDecodeOn.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeActivationPreflightSideEffectFree === true &&
      switchOnDryRunOnResultConsumptionOnReceiptDecodeOn.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeActivationPreflight
        ?.safety?.sanitized === true &&
      switchOnDryRunOnResultConsumptionOnReceiptDecodeOn.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeActivationPreflight
        ?.safety?.productionReleaseAuthorized === false &&
      switchOnDryRunOnResultConsumptionOnReceiptDecodeOn.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeActivationPreflight
        ?.safety?.productionRelease === false &&
      switchOnDryRunOnResultConsumptionOnReceiptDecodeOn.productionRelease === false,

    crpFulfillClientAdapterResultConsumptionReceiptDecodeNoopDecoderInactiveUntilActivationPreflightObserved:
      switchOffDryRunOff.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeNoopDecoderRequired === false &&
      switchOffDryRunOff.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeNoopDecoderObserved === false &&
      switchOffDryRunOff.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeNoopDecoderStatus === "inactive" &&
      switchOffDryRunOff.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeNoopDecoder === null &&
      switchOffDryRunOn.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeNoopDecoderRequired === false &&
      switchOffDryRunOn.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeNoopDecoderObserved === false &&
      switchOffDryRunOn.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeNoopDecoderStatus === "inactive" &&
      switchOffDryRunOn.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeNoopDecoder === null &&
      switchOnDryRunOff.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeNoopDecoderRequired === false &&
      switchOnDryRunOff.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeNoopDecoderObserved === false &&
      switchOnDryRunOff.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeNoopDecoderStatus === "inactive" &&
      switchOnDryRunOff.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeNoopDecoder === null &&
      switchOnDryRunOn.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeNoopDecoderRequired === false &&
      switchOnDryRunOn.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeNoopDecoderObserved === false &&
      switchOnDryRunOn.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeNoopDecoderStatus === "inactive" &&
      switchOnDryRunOn.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeNoopDecoder === null &&
      switchOnDryRunOnResultConsumptionOn.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeNoopDecoderRequired === false &&
      switchOnDryRunOnResultConsumptionOn.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeNoopDecoderObserved === false &&
      switchOnDryRunOnResultConsumptionOn.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeNoopDecoderStatus === "inactive" &&
      switchOnDryRunOnResultConsumptionOn.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeNoopDecoder === null,

    crpFulfillClientAdapterResultConsumptionReceiptDecodeNoopDecoderObservedAfterActivationPreflight:
      switchOnDryRunOnResultConsumptionOnReceiptDecodeOn.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeActivationPreflightRequired === true &&
      switchOnDryRunOnResultConsumptionOnReceiptDecodeOn.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeActivationPreflightObserved === true &&
      switchOnDryRunOnResultConsumptionOnReceiptDecodeOn.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeActivationPreflightStatus === "preflight_ready" &&
      switchOnDryRunOnResultConsumptionOnReceiptDecodeOn.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeNoopDecoderRequired === true &&
      switchOnDryRunOnResultConsumptionOnReceiptDecodeOn.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeNoopDecoderObserved === true &&
      switchOnDryRunOnResultConsumptionOnReceiptDecodeOn.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeNoopDecoderReady === false &&
      switchOnDryRunOnResultConsumptionOnReceiptDecodeOn.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeNoopDecoderStatus === "unavailable" &&
      switchOnDryRunOnResultConsumptionOnReceiptDecodeOn.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeNoopDecoderReason === "production_release_crp_fulfill_client_adapter_result_consumption_receipt_decode_noop_decoder_unavailable" &&
      switchOnDryRunOnResultConsumptionOnReceiptDecodeOn.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeNoopDecoderBlockedBy === "production_release_crp_fulfill_client_adapter_result_consumption_receipt_decode_noop_decoder_unavailable" &&
      switchOnDryRunOnResultConsumptionOnReceiptDecodeOn.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeNoopDecoder?.contract === "phase3.productionRelease.crpFulfillClientAdapter.resultConsumptionReceiptDecodeNoopDecoder.v1" &&
      switchOnDryRunOnResultConsumptionOnReceiptDecodeOn.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeNoopDecoder?.mode === "no_op",

    crpFulfillClientAdapterResultConsumptionReceiptDecodeNoopDecoderReportsImplementationUnavailable:
      switchOnDryRunOnResultConsumptionOnReceiptDecodeOn.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeNoopDecoder
        ?.source?.activationPreflightStatus === "preflight_ready" &&
      switchOnDryRunOnResultConsumptionOnReceiptDecodeOn.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeNoopDecoder
        ?.source?.activationPreflightContract ===
        "phase3.productionRelease.crpFulfillClientAdapter.resultConsumptionReceiptDecodeActivationPreflight.v1" &&
      switchOnDryRunOnResultConsumptionOnReceiptDecodeOn.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeNoopDecoder
        ?.source?.enablementGateStatus ===
        switchOnDryRunOnResultConsumptionOnReceiptDecodeOn.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeEnablementGateStatus &&
      switchOnDryRunOnResultConsumptionOnReceiptDecodeOn.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeNoopDecoder
        ?.decoder?.implementationRequired === true &&
      switchOnDryRunOnResultConsumptionOnReceiptDecodeOn.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeNoopDecoder
        ?.decoder?.implementationAvailable === false &&
      switchOnDryRunOnResultConsumptionOnReceiptDecodeOn.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeNoopDecoder
        ?.decoder?.invocationAllowed === false &&
      switchOnDryRunOnResultConsumptionOnReceiptDecodeOn.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeNoopDecoder
        ?.decoder?.invocationObserved === false &&
      switchOnDryRunOnResultConsumptionOnReceiptDecodeOn.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeNoopDecoder
        ?.decoder?.receiptJwsAccepted === false &&
      switchOnDryRunOnResultConsumptionOnReceiptDecodeOn.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeNoopDecoder
        ?.decoder?.receiptPayloadAccepted === false,

    crpFulfillClientAdapterResultConsumptionReceiptDecodeNoopDecoderStillBlocksAllExecution:
      switchOnDryRunOnResultConsumptionOnReceiptDecodeOn.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeNoopDecoderAllowsReceiptDecode === false &&
      switchOnDryRunOnResultConsumptionOnReceiptDecodeOn.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeNoopDecoderAllowsReceiptPayloadParse === false &&
      switchOnDryRunOnResultConsumptionOnReceiptDecodeOn.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeNoopDecoderAllowsFinalizedSettlementVerification === false &&
      switchOnDryRunOnResultConsumptionOnReceiptDecodeOn.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeNoopDecoderAllowsTupleBinding === false &&
      switchOnDryRunOnResultConsumptionOnReceiptDecodeOn.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeNoopDecoderAllowsReplayCheck === false &&
      switchOnDryRunOnResultConsumptionOnReceiptDecodeOn.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeNoopDecoderAllowsCrpFulfill === false &&
      switchOnDryRunOnResultConsumptionOnReceiptDecodeOn.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeNoopDecoderAllowsProductionRelease === false &&
      switchOnDryRunOnResultConsumptionOnReceiptDecodeOn.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeNoopDecoderReceiptMaterialObserved === false &&
      switchOnDryRunOnResultConsumptionOnReceiptDecodeOn.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeNoopDecoderReceiptMaterialHandled === false &&
      switchOnDryRunOnResultConsumptionOnReceiptDecodeOn.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeNoopDecoderReceiptMaterialConsumed === false &&
      switchOnDryRunOnResultConsumptionOnReceiptDecodeOn.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeNoopDecoderReceiptDecoded === false &&
      switchOnDryRunOnResultConsumptionOnReceiptDecodeOn.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeNoopDecoderReceiptPayloadParsed === false &&
      switchOnDryRunOnResultConsumptionOnReceiptDecodeOn.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeNoopDecoderFinalizedSettlementVerified === false &&
      switchOnDryRunOnResultConsumptionOnReceiptDecodeOn.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeNoopDecoderTupleBindingVerified === false &&
      switchOnDryRunOnResultConsumptionOnReceiptDecodeOn.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeNoopDecoderReplayTouched === false &&
      switchOnDryRunOnResultConsumptionOnReceiptDecodeOn.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeNoopDecoderAdapterInvoked === false &&
      switchOnDryRunOnResultConsumptionOnReceiptDecodeOn.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeNoopDecoderExternalCallAttempted === false &&
      switchOnDryRunOnResultConsumptionOnReceiptDecodeOn.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeNoopDecoderCrpCalled === false &&
      switchOnDryRunOnResultConsumptionOnReceiptDecodeOn.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeNoopDecoderCrpFulfillCalled === false &&
      switchOnDryRunOnResultConsumptionOnReceiptDecodeOn.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeNoopDecoderRawProofPrinted === false &&
      switchOnDryRunOnResultConsumptionOnReceiptDecodeOn.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeNoopDecoderRawReceiptPrinted === false &&
      switchOnDryRunOnResultConsumptionOnReceiptDecodeOn.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeNoopDecoderSideEffectFree === true &&
      switchOnDryRunOnResultConsumptionOnReceiptDecodeOn.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeNoopDecoder
        ?.safety?.sanitized === true &&
      switchOnDryRunOnResultConsumptionOnReceiptDecodeOn.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeNoopDecoder
        ?.safety?.productionReleaseAuthorized === false &&
    ...
    crpFulfillClientAdapterResultConsumptionReceiptDecodeMetadataBoundaryInactiveUntilNoopDecoderObserved:
      switchOffDryRunOff.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeMetadataBoundaryRequired === false &&
      switchOffDryRunOff.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeMetadataBoundaryObserved === false &&
      switchOffDryRunOff.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeMetadataBoundaryStatus === "inactive" &&
      switchOffDryRunOff.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeMetadataBoundary === null &&
      switchOffDryRunOn.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeMetadataBoundaryRequired === false &&
      switchOffDryRunOn.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeMetadataBoundaryObserved === false &&
      switchOffDryRunOn.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeMetadataBoundaryStatus === "inactive" &&
      switchOffDryRunOn.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeMetadataBoundary === null &&
      switchOnDryRunOff.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeMetadataBoundaryRequired === false &&
      switchOnDryRunOff.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeMetadataBoundaryObserved === false &&
      switchOnDryRunOff.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeMetadataBoundaryStatus === "inactive" &&
      switchOnDryRunOff.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeMetadataBoundary === null &&
      switchOnDryRunOn.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeMetadataBoundaryRequired === false &&
      switchOnDryRunOn.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeMetadataBoundaryObserved === false &&
      switchOnDryRunOn.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeMetadataBoundaryStatus === "inactive" &&
      switchOnDryRunOn.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeMetadataBoundary === null &&
      switchOnDryRunOnResultConsumptionOn.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeMetadataBoundaryRequired === false &&
      switchOnDryRunOnResultConsumptionOn.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeMetadataBoundaryObserved === false &&
      switchOnDryRunOnResultConsumptionOn.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeMetadataBoundaryStatus === "inactive" &&
      switchOnDryRunOnResultConsumptionOn.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeMetadataBoundary === null,

    crpFulfillClientAdapterResultConsumptionReceiptDecodeMetadataBoundaryObservedAfterNoopDecoder:
      switchOnDryRunOnResultConsumptionOnReceiptDecodeOn.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeNoopDecoderRequired === true &&
      switchOnDryRunOnResultConsumptionOnReceiptDecodeOn.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeNoopDecoderObserved === true &&
      switchOnDryRunOnResultConsumptionOnReceiptDecodeOn.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeNoopDecoderStatus === "unavailable" &&
      switchOnDryRunOnResultConsumptionOnReceiptDecodeOn.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeMetadataBoundaryRequired === true &&
      switchOnDryRunOnResultConsumptionOnReceiptDecodeOn.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeMetadataBoundaryObserved === true &&
      switchOnDryRunOnResultConsumptionOnReceiptDecodeOn.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeMetadataBoundaryReady === false &&
      switchOnDryRunOnResultConsumptionOnReceiptDecodeOn.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeMetadataBoundaryStatus === "preflight_ready" &&
      switchOnDryRunOnResultConsumptionOnReceiptDecodeOn.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeMetadataBoundaryReason === "production_release_crp_fulfill_client_adapter_result_consumption_receipt_decode_metadata_boundary_preflight_ready" &&
      switchOnDryRunOnResultConsumptionOnReceiptDecodeOn.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeMetadataBoundaryBlockedBy === null &&
      switchOnDryRunOnResultConsumptionOnReceiptDecodeOn.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeMetadataBoundary?.contract === "phase3.productionRelease.crpFulfillClientAdapter.resultConsumptionReceiptDecodeMetadataBoundary.v1" &&
      switchOnDryRunOnResultConsumptionOnReceiptDecodeOn.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeMetadataBoundary?.mode === "contract_only",

    crpFulfillClientAdapterResultConsumptionReceiptDecodeMetadataBoundaryDeclaresMetadataOnlyAndProhibitedReceiptMaterial:
      switchOnDryRunOnResultConsumptionOnReceiptDecodeOn.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeMetadataBoundary
        ?.source?.noopDecoderStatus === "unavailable" &&
      switchOnDryRunOnResultConsumptionOnReceiptDecodeOn.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeMetadataBoundary
        ?.source?.noopDecoderContract ===
        "phase3.productionRelease.crpFulfillClientAdapter.resultConsumptionReceiptDecodeNoopDecoder.v1" &&
      switchOnDryRunOnResultConsumptionOnReceiptDecodeOn.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeMetadataBoundary
        ?.decoderInput?.futureDecoderInputRequired === true &&
      switchOnDryRunOnResultConsumptionOnReceiptDecodeOn.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeMetadataBoundary
        ?.decoderInput?.metadataOnly === true &&
      switchOnDryRunOnResultConsumptionOnReceiptDecodeOn.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeMetadataBoundary
        ?.decoderInput?.inputObjectBuilt === false &&
      switchOnDryRunOnResultConsumptionOnReceiptDecodeOn.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeMetadataBoundary
        ?.decoderInput?.decoderInvocationAllowed === false &&
      switchOnDryRunOnResultConsumptionOnReceiptDecodeOn.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeMetadataBoundary
        ?.decoderInput?.decoderInvocationObserved === false &&
      switchOnDryRunOnResultConsumptionOnReceiptDecodeOn.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeMetadataBoundary
        ?.decoderInput?.allowedMetadataCategories?.includes("nonceBinding") === true &&
      switchOnDryRunOnResultConsumptionOnReceiptDecodeOn.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeMetadataBoundary
        ?.decoderInput?.allowedMetadataCategories?.includes("upstreamGateContext") === true &&
      switchOnDryRunOnResultConsumptionOnReceiptDecodeOn.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeMetadataBoundary
        ?.decoderInput?.prohibitedReceiptMaterialCategories?.includes("receiptJws") === true &&
      switchOnDryRunOnResultConsumptionOnReceiptDecodeOn.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeMetadataBoundary
        ?.decoderInput?.prohibitedReceiptMaterialCategories?.includes("receiptPayload") === true &&
      switchOnDryRunOnResultConsumptionOnReceiptDecodeOn.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeMetadataBoundary
        ?.decoderInput?.prohibitedReceiptMaterialCategories?.includes("receiptBytes") === true &&
      switchOnDryRunOnResultConsumptionOnReceiptDecodeOn.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeMetadataBoundary
        ?.decoderInput?.prohibitedReceiptMaterialCategories?.includes("receiptObject") === true &&
      switchOnDryRunOnResultConsumptionOnReceiptDecodeOn.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeMetadataBoundary
        ?.decoderInput?.prohibitedReceiptMaterialCategories?.includes("transactionHash") === true &&
      switchOnDryRunOnResultConsumptionOnReceiptDecodeOn.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeMetadataBoundary
        ?.decoderInput?.prohibitedReceiptMaterialCategories?.includes("settlementFields") === true &&
      switchOnDryRunOnResultConsumptionOnReceiptDecodeOn.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeMetadataBoundary
        ?.decoderInput?.prohibitedReceiptMaterialCategories?.includes("replayKey") === true &&
      switchOnDryRunOnResultConsumptionOnReceiptDecodeOn.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeMetadataBoundary
        ?.decoderInput?.receiptJwsAccepted === false &&
      switchOnDryRunOnResultConsumptionOnReceiptDecodeOn.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeMetadataBoundary
        ?.decoderInput?.receiptPayloadAccepted === false &&
      switchOnDryRunOnResultConsumptionOnReceiptDecodeOn.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeMetadataBoundary
        ?.decoderInput?.receiptBytesAccepted === false &&
      switchOnDryRunOnResultConsumptionOnReceiptDecodeOn.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeMetadataBoundary
        ?.decoderInput?.receiptObjectAccepted === false &&
      switchOnDryRunOnResultConsumptionOnReceiptDecodeOn.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeMetadataBoundary
        ?.decoderInput?.transactionHashAccepted === false,

    crpFulfillClientAdapterResultConsumptionReceiptDecodeMetadataBoundaryStillBlocksAllExecution:
      switchOnDryRunOnResultConsumptionOnReceiptDecodeOn.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeMetadataBoundaryAllowsReceiptMaterialObservation === false &&
      switchOnDryRunOnResultConsumptionOnReceiptDecodeOn.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeMetadataBoundaryAllowsReceiptMaterialHandling === false &&
      switchOnDryRunOnResultConsumptionOnReceiptDecodeOn.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeMetadataBoundaryAllowsReceiptMaterialConsumption === false &&
      switchOnDryRunOnResultConsumptionOnReceiptDecodeOn.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeMetadataBoundaryAllowsReceiptDecode === false &&
      switchOnDryRunOnResultConsumptionOnReceiptDecodeOn.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeMetadataBoundaryAllowsReceiptPayloadParse === false &&
      switchOnDryRunOnResultConsumptionOnReceiptDecodeOn.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeMetadataBoundaryAllowsFinalizedSettlementVerification === false &&
      switchOnDryRunOnResultConsumptionOnReceiptDecodeOn.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeMetadataBoundaryAllowsTupleBinding === false &&
      switchOnDryRunOnResultConsumptionOnReceiptDecodeOn.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeMetadataBoundaryAllowsReplayCheck === false &&
      switchOnDryRunOnResultConsumptionOnReceiptDecodeOn.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeMetadataBoundaryAllowsCrpFulfill === false &&
      switchOnDryRunOnResultConsumptionOnReceiptDecodeOn.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeMetadataBoundaryAllowsProductionRelease === false &&
      switchOnDryRunOnResultConsumptionOnReceiptDecodeOn.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeMetadataBoundaryReceiptMaterialObserved === false &&
      switchOnDryRunOnResultConsumptionOnReceiptDecodeOn.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeMetadataBoundaryReceiptMaterialHandled === false &&
      switchOnDryRunOnResultConsumptionOnReceiptDecodeOn.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeMetadataBoundaryReceiptMaterialConsumed === false &&
      switchOnDryRunOnResultConsumptionOnReceiptDecodeOn.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeMetadataBoundaryReceiptDecoded === false &&
      switchOnDryRunOnResultConsumptionOnReceiptDecodeOn.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeMetadataBoundaryReceiptPayloadParsed === false &&
      switchOnDryRunOnResultConsumptionOnReceiptDecodeOn.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeMetadataBoundaryFinalizedSettlementVerified === false &&
      switchOnDryRunOnResultConsumptionOnReceiptDecodeOn.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeMetadataBoundaryTupleBindingVerified === false &&
      switchOnDryRunOnResultConsumptionOnReceiptDecodeOn.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeMetadataBoundaryReplayTouched === false &&
      switchOnDryRunOnResultConsumptionOnReceiptDecodeOn.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeMetadataBoundaryAdapterInvoked === false &&
      switchOnDryRunOnResultConsumptionOnReceiptDecodeOn.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeMetadataBoundaryExternalCallAttempted === false &&
      switchOnDryRunOnResultConsumptionOnReceiptDecodeOn.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeMetadataBoundaryCrpCalled === false &&
      switchOnDryRunOnResultConsumptionOnReceiptDecodeOn.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeMetadataBoundaryCrpFulfillCalled === false &&
      switchOnDryRunOnResultConsumptionOnReceiptDecodeOn.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeMetadataBoundaryRawProofPrinted === false &&
      switchOnDryRunOnResultConsumptionOnReceiptDecodeOn.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeMetadataBoundaryRawReceiptPrinted === false &&
      switchOnDryRunOnResultConsumptionOnReceiptDecodeOn.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeMetadataBoundarySideEffectFree === true &&
      switchOnDryRunOnResultConsumptionOnReceiptDecodeOn.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeMetadataBoundary
        ?.safety?.sanitized === true &&
      switchOnDryRunOnResultConsumptionOnReceiptDecodeOn.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeMetadataBoundary
        ?.safety?.productionReleaseAuthorized === false &&
  ...
  assert.equal(
    summary.crpFulfillClientAdapterResultConsumptionReceiptDecodeReadinessGateInactiveUntilReceiptDecodeDryRunAuditObserved,
    true,
  );
  assert.equal(
    summary.crpFulfillClientAdapterResultConsumptionReceiptDecodeReadinessGateObservedAfterReceiptDecodeDryRunAudit,
    true,
  );
  assert.equal(
    summary.crpFulfillClientAdapterResultConsumptionReceiptDecodeReadinessGatePreservesDryRunAuditContext,
    true,
  );
  assert.equal(
    summary.crpFulfillClientAdapterResultConsumptionReceiptDecodeReadinessGateRecordsReadinessChecklist,
    true,
  );
    assert.equal(
      summary.crpFulfillClientAdapterResultConsumptionReceiptDecodeReadinessGateBlocksReceiptDecode,
      true,
    );
  assert.equal(
    summary.crpFulfillClientAdapterResultConsumptionReceiptDecodeReadinessGateDoesNotObserveReceiptMaterial,
    true,
  );
  assert.equal(
    summary.crpFulfillClientAdapterResultConsumptionReceiptDecodeReadinessGateDoesNotHandleOrConsumeReceiptMaterial,
    true,
  );
  assert.equal(
    summary.crpFulfillClientAdapterResultConsumptionReceiptDecodeReadinessGateDoesNotDecodeOrParseReceipt,
    true,
  );
  assert.equal(
    summary.crpFulfillClientAdapterResultConsumptionReceiptDecodeReadinessGateDoesNotVerifySettlementOrTuple,
    true,
  );
  assert.equal(
    summary.crpFulfillClientAdapterResultConsumptionReceiptDecodeReadinessGateDoesNotTouchReplay,
    true,
  );
  assert.equal(
    summary.crpFulfillClientAdapterResultConsumptionReceiptDecodeReadinessGateDoesNotAuthorizeProductionRelease,
    true,
  );
  assert.equal(
    summary.crpFulfillClientAdapterResultConsumptionReceiptDecodeReadinessGateSideEffectFree,
    true,
  );
  assert.equal(
    summary.crpFulfillClientAdapterResultConsumptionReceiptDecodeEnablementGateInactiveUntilReadinessGateObserved,
    true,
  );
  assert.equal(
    summary.crpFulfillClientAdapterResultConsumptionReceiptDecodeEnablementGateBlockedWhenFlagOff,
    true,
  );
  assert.equal(
    summary.crpFulfillClientAdapterResultConsumptionReceiptDecodeEnablementGateEnabledWhenFlagOnButNotReady,
    true,
  );
  assert.equal(
    summary.crpFulfillClientAdapterResultConsumptionReceiptDecodeEnablementGateStillBlocksAllExecution,
    true,
  );
  assert.equal(
    summary.crpFulfillClientAdapterResultConsumptionReceiptDecodeActivationPreflightInactiveUntilEnablementGateEnabled,
    true,
  );
  assert.equal(
    summary.crpFulfillClientAdapterResultConsumptionReceiptDecodeActivationPreflightObservedAfterEnablementGateEnabled,
    true,
  );
  assert.equal(
    summary.crpFulfillClientAdapterResultConsumptionReceiptDecodeActivationPreflightPreservesEnablementContextAndRecordsRequirements,
    true,
  );
  assert.equal(
    summary.crpFulfillClientAdapterResultConsumptionReceiptDecodeActivationPreflightStillBlocksAllExecution,
    true,
  );
  assert.equal(
    summary.crpFulfillClientAdapterResultConsumptionReceiptDecodeNoopDecoderInactiveUntilActivationPreflightObserved,
    true,
  );
  assert.equal(
    summary.crpFulfillClientAdapterResultConsumptionReceiptDecodeNoopDecoderObservedAfterActivationPreflight,
    true,
  );
  assert.equal(
    summary.crpFulfillClientAdapterResultConsumptionReceiptDecodeNoopDecoderReportsImplementationUnavailable,
    true,
  );
  assert.equal(
    summary.crpFulfillClientAdapterResultConsumptionReceiptDecodeNoopDecoderStillBlocksAllExecution,
    true,
  );
  assert.equal(
    summary.crpFulfillClientAdapterResultConsumptionReceiptDecodeMetadataBoundaryInactiveUntilNoopDecoderObserved,
    true,
  );
  assert.equal(
    summary.crpFulfillClientAdapterResultConsumptionReceiptDecodeMetadataBoundaryObservedAfterNoopDecoder,
    true,
  );
  assert.equal(
    summary.crpFulfillClientAdapterResultConsumptionReceiptDecodeMetadataBoundaryDeclaresMetadataOnlyAndProhibitedReceiptMaterial,
    true,
  );
  assert.equal(
    summary.crpFulfillClientAdapterResultConsumptionReceiptDecodeMetadataBoundaryStillBlocksAllExecution,
    true,
  );
    ...
    JSON.stringify(
      {
        ...
        noopDecoderInactiveUntilActivationPreflightObserved:
          summary.crpFulfillClientAdapterResultConsumptionReceiptDecodeNoopDecoderInactiveUntilActivationPreflightObserved,
        noopDecoderObservedAfterActivationPreflight:
          summary.crpFulfillClientAdapterResultConsumptionReceiptDecodeNoopDecoderObservedAfterActivationPreflight,
        noopDecoderReportsImplementationUnavailable:
          summary.crpFulfillClientAdapterResultConsumptionReceiptDecodeNoopDecoderReportsImplementationUnavailable,
        noopDecoderStillBlocksAllExecution:
          summary.crpFulfillClientAdapterResultConsumptionReceiptDecodeNoopDecoderStillBlocksAllExecution,
        metadataBoundaryInactiveUntilNoopDecoderObserved:
          summary.crpFulfillClientAdapterResultConsumptionReceiptDecodeMetadataBoundaryInactiveUntilNoopDecoderObserved,
        metadataBoundaryObservedAfterNoopDecoder:
          summary.crpFulfillClientAdapterResultConsumptionReceiptDecodeMetadataBoundaryObservedAfterNoopDecoder,
        metadataBoundaryDeclaresMetadataOnlyAndProhibitedReceiptMaterial:
          summary.crpFulfillClientAdapterResultConsumptionReceiptDecodeMetadataBoundaryDeclaresMetadataOnlyAndProhibitedReceiptMaterial,
        metadataBoundaryStillBlocksAllExecution:
          summary.crpFulfillClientAdapterResultConsumptionReceiptDecodeMetadataBoundaryStillBlocksAllExecution,
        productionReleaseStillFalse: summary.productionReleaseStillFalse,
        crpFulfillStillFalse: summary.crpFulfillStillFalse,
        rawProofAndReceiptNotPrinted: summary.rawProofAndReceiptNotPrinted,
        behaviorStillSideEffectFree: summary.behaviorStillSideEffectFree,
      },
      null,
      2,
    ),
