#!/usr/bin/env node
/**
 * scripts/ci_phase3_gateway_test_release_guard.ts
 *
 * PR #130A regression harness.
 *
 * Proves PHASE3_GATEWAY_RELEASE_ENABLED=true alone is not sufficient.
 *
 * Required behavior:
 *
 *   eligible + finalized x402 receipt -> release decision authorized
 *   PHASE3_GATEWAY_RELEASE_ENABLED=true
 *   PHASE3_GATEWAY_TEST_RELEASE_ONLY unset/false
...
const GATEWAY_PORT = Number(process.env.PHASE3_GATEWAY_TEST_RELEASE_GUARD_PORT || 3067);
const BASE = baseUrlForPort(GATEWAY_PORT);
const LABEL = "phase3:gateway-test-release-guard-test";
...
async function main() {
  ...
  if (String(process.env.PHASE3_GATEWAY_TEST_RELEASE_ONLY ?? "").toLowerCase() === "true") {
    throw new Error("PHASE3_GATEWAY_TEST_RELEASE_ONLY must not be true for this guard harness.");
  }
      ...
      JSON.stringify(
        {
          ...
          releaseFlagAloneInsufficient: health.phase3.gatewayReleaseEnabled === true && health.phase3.gatewayTestReleaseOnly === false,
          releaseBlockedWithoutTestOnlyGuard: true,
