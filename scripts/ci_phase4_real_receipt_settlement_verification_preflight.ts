#!/usr/bin/env node
/**
 * PR 4-5 — Phase 4 real receipt settlement verification preflight.
 *
 * Receipt settlement verification preflight checkpoint.
 *
 * This validates the Phase 4 receipt settlement verification preflight on top of the signature verification preflight:
...
const LABEL = "phase4:real-receipt-settlement-verification-preflight-test";
const GATEWAY_PORT = Number(process.env.PHASE4_REAL_RECEIPT_SETTLEMENT_VERIFICATION_PREFLIGHT_PORT || 3126);
const CRP_PORT = Number(process.env.PHASE4_REAL_RECEIPT_SETTLEMENT_VERIFICATION_PREFLIGHT_CRP_PORT || 8126);
...
async function syntheticReceiptJws(body: any, privateKey: any): Promise<string> {
  ...
  return await new SignJWT({
    ...
    sub: "phase4-real-receipt-settlement-verification-preflight",
    receiptVersion: "phase4.synthetic.settlement-verification-preflight.v1",
...
async function main() {
  ...
  try {
    ...
    assert.equal(decodePreflight?.payload?.receiptVersion, "phase4.synthetic.settlement-verification-preflight.v1");
    ...
    assert.equal(signaturePreflight?.payload?.receiptVersion, "phase4.synthetic.settlement-verification-preflight.v1");
