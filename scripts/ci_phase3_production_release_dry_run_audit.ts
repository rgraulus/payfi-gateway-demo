#!/usr/bin/env node
/**
 * PR #178 — Phase 3 production release dry-run audit event.
 *
 * This harness proves the dry-run / would-execute audit boundary.
 *
 * - switch OFF + dry-run OFF => blocked by production switch
 * - switch OFF + dry-run ON  => still blocked by production switch
 * - switch ON  + dry-run OFF => blocked by production release execution preflight
 * - switch ON  + dry-run ON  => emits would-execute audit metadata only
...
const LABEL = "phase3:production-release-dry-run-audit-test";

const SWITCH_OFF_GATEWAY_PORT = Number(process.env.PHASE3_PROD_DRY_RUN_AUDIT_OFF_PORT || 3098);
const SWITCH_OFF_JWKS_PORT = Number(process.env.PHASE3_PROD_DRY_RUN_AUDIT_OFF_JWKS_PORT || 8108);

const SWITCH_ON_GATEWAY_PORT = Number(process.env.PHASE3_PROD_DRY_RUN_AUDIT_ON_PORT || 3099);
const SWITCH_ON_JWKS_PORT = Number(process.env.PHASE3_PROD_DRY_RUN_AUDIT_ON_JWKS_PORT || 8109);
...
async function main() {
  ...
  const summary = {
    ok: true,
    harness: "phase3.productionReleaseDryRunAudit.v1",

    dryRunFlagDefaultsOff:
      switchOffDryRunOff.gatewayProductionReleaseDryRunEnabled === false,

    switchOffAlwaysBlocksProductionPath:
      switchOffDryRunOff.productionReleaseBlockedBy === "production_release_switch_disabled" &&
      switchOffDryRunOn.productionReleaseBlockedBy === "production_release_switch_disabled" &&
      switchOffDryRunOff.productionReleaseDryRun === false &&
      switchOffDryRunOn.productionReleaseDryRun === false &&
      switchOffDryRunOff.productionReleaseWouldExecute === false &&
      switchOffDryRunOn.productionReleaseWouldExecute === false,

    switchOnDryRunOffPreservesExecutionDisabledBoundary:
      switchOnDryRunOff.productionReleaseCandidate === true &&
      switchOnDryRunOff.productionReleaseEligible === true &&
      switchOnDryRunOff.canonicalReleasePersistenceRequired === true &&
      switchOnDryRunOff.canonicalReleasePersistenceReady === true &&
      switchOnDryRunOff.canonicalReleasePersisted === true &&
      switchOnDryRunOff.productionReleaseExecutionPreflightRequired === true &&
      switchOnDryRunOff.productionReleaseExecutionPreflightReady === false &&
      switchOnDryRunOff.productionReleaseExecutionMode === "disabled" &&
      switchOnDryRunOff.productionReleaseExecutionBlockedBy === "production_release_execution_disabled" &&
      switchOnDryRunOff.productionReleaseDryRun === false &&
      switchOnDryRunOff.productionReleaseWouldExecute === false &&
      switchOnDryRunOff.productionReleaseDryRunAuditEvent === false &&
      switchOnDryRunOff.productionRelease === false,

    switchOnDryRunOnEmitsWouldExecuteAuditOnly:
      switchOnDryRunOn.productionReleaseCandidate === true &&
      switchOnDryRunOn.productionReleaseEligible === true &&
      switchOnDryRunOn.canonicalReleasePersistenceRequired === true &&
      switchOnDryRunOn.canonicalReleasePersistenceReady === true &&
      switchOnDryRunOn.canonicalReleasePersisted === true &&
      switchOnDryRunOn.productionReleaseExecutionPreflightRequired === true &&
      switchOnDryRunOn.productionReleaseExecutionPreflightReady === true &&
      switchOnDryRunOn.productionReleaseExecutionMode === "dry_run" &&
      switchOnDryRunOn.productionReleaseExecutionBlockedBy === null &&
      switchOnDryRunOn.productionReleaseDryRun === true &&
      switchOnDryRunOn.productionReleaseWouldExecute === true &&
      switchOnDryRunOn.productionReleaseDryRunAuditEvent === true &&
      switchOnDryRunOn.productionReleaseDryRunReason === "production_release_would_execute" &&
      switchOnDryRunOn.productionRelease === false,

    testOnlyReleaseStillAllowed:
      switchOffDryRunOff.releaseStatus === 200 &&
      switchOffDryRunOn.releaseStatus === 200 &&
      switchOnDryRunOff.releaseStatus === 200 &&
      switchOnDryRunOn.releaseStatus === 200 &&
      switchOffDryRunOff.paymentResponseEmitted === true &&
      switchOffDryRunOn.paymentResponseEmitted === true &&
      switchOnDryRunOff.paymentResponseEmitted === true &&
      switchOnDryRunOn.paymentResponseEmitted === true &&
      switchOffDryRunOff.resourceReleased === true &&
      switchOffDryRunOn.resourceReleased === true &&
      switchOnDryRunOff.resourceReleased === true &&
      switchOnDryRunOn.resourceReleased === true,
  ...
  assert.equal(summary.dryRunFlagDefaultsOff, true);
  assert.equal(summary.switchOffAlwaysBlocksProductionPath, true);
  assert.equal(summary.switchOnDryRunOffPreservesExecutionDisabledBoundary, true);
  assert.equal(summary.switchOnDryRunOnEmitsWouldExecuteAuditOnly, true);
