#!/usr/bin/env node
/**
 * PR #196 — Phase 3 production release CRP fulfill client adapter handoff readiness boundary.
 *
 * This harness proves the Gateway builds a sanitized CRP fulfill client adapter input
 * after the adapter contract boundary, while keeping execution side-effect-free.
 *
...
const LABEL = "phase3:production-release-crp-fulfill-client-adapter-handoff-readiness-test";

const SWITCH_OFF_GATEWAY_PORT = Number(process.env.PHASE3_PROD_CRP_FULFILL_CLIENT_ADAPTER_HANDOFF_READINESS_OFF_PORT || 3112);
const SWITCH_OFF_JWKS_PORT = Number(process.env.PHASE3_PROD_CRP_FULFILL_CLIENT_ADAPTER_HANDOFF_READINESS_OFF_JWKS_PORT || 8122);

const SWITCH_ON_GATEWAY_PORT = Number(process.env.PHASE3_PROD_CRP_FULFILL_CLIENT_ADAPTER_HANDOFF_READINESS_ON_PORT || 3113);
const SWITCH_ON_JWKS_PORT = Number(process.env.PHASE3_PROD_CRP_FULFILL_CLIENT_ADAPTER_HANDOFF_READINESS_ON_JWKS_PORT || 8123);
...
async function main() {
  ...
  const summary = {
    ok: true,
    harness: "phase3.productionReleaseCrpFulfillClientAdapterHandoffReadiness.v1",
    ...
    crpFulfillRequestDraftSideEffectFree:
      ...
      crpFulfillClientAdapterDryRunInvocationReceiptSideEffectFree:
        ...
        switchOnDryRunOn.productionReleaseCrpFulfillClientAdapterDryRunInvocationReceipt?.safety
          ?.sideEffectFree === true,
      crpFulfillClientAdapterHandoffReadinessObservedAfterReceipt:
        switchOnDryRunOn.productionReleaseCrpFulfillClientAdapterDryRunInvocationReceiptEmitted === true &&
        switchOnDryRunOn.productionReleaseCrpFulfillClientAdapterHandoffReadinessRequired === true &&
        switchOnDryRunOn.productionReleaseCrpFulfillClientAdapterHandoffReadinessReady === true &&
        switchOnDryRunOn.productionReleaseCrpFulfillClientAdapterHandoffReadinessContract ===
          "phase3.productionRelease.crpFulfillClientAdapter.handoffReadiness.v1" &&
        switchOnDryRunOn.productionReleaseCrpFulfillClientAdapterHandoffReadinessStatus === "ready" &&
        switchOnDryRunOn.productionReleaseCrpFulfillClientAdapterHandoffReadinessReason ===
          "production_release_crp_fulfill_client_adapter_handoff_ready",
      crpFulfillClientAdapterHandoffReadinessPreservesReceiptContext:
        switchOnDryRunOn.productionReleaseCrpFulfillClientAdapterHandoffReadiness?.source
          ?.receiptContract ===
          "phase3.productionRelease.crpFulfillClientAdapter.dryRunInvocationReceipt.v1" &&
        switchOnDryRunOn.productionReleaseCrpFulfillClientAdapterHandoffReadiness?.source
          ?.receiptReason ===
        ...
        switchOnDryRunOn.productionReleaseCrpFulfillClientAdapterHandoffReadiness?.input?.contract ===
          "phase3.productionRelease.crpFulfillClientAdapter.input.v1" &&
        switchOnDryRunOn.productionReleaseCrpFulfillClientAdapterHandoffReadiness?.input?.ready === true,
      crpFulfillClientAdapterHandoffReadinessBlocksExecution:
        switchOnDryRunOn.productionReleaseCrpFulfillClientAdapterHandoffReadinessBlockedBy ===
          "production_release_crp_fulfill_client_adapter_execution_not_enabled" &&
        switchOnDryRunOn.productionReleaseCrpFulfillClientAdapterHandoffReadinessAllowsCrpFulfill ===
          false &&
        switchOnDryRunOn.productionReleaseCrpFulfillClientAdapterHandoffReadinessAllowsProductionRelease ===
          false &&
        switchOnDryRunOn.productionReleaseCrpFulfillClientAdapterHandoffReadiness?.blockedBy ===
          "production_release_crp_fulfill_client_adapter_execution_not_enabled",
      crpFulfillClientAdapterHandoffReadinessSideEffectFree:
        switchOnDryRunOn.productionReleaseCrpFulfillClientAdapterHandoffReadinessAdapterInvoked ===
          false &&
        switchOnDryRunOn.productionReleaseCrpFulfillClientAdapterHandoffReadinessExternalCallAttempted ===
          false &&
        switchOnDryRunOn.productionReleaseCrpFulfillClientAdapterHandoffReadinessCrpCalled === false &&
        switchOnDryRunOn.productionReleaseCrpFulfillClientAdapterHandoffReadinessCrpFulfillCalled ===
          false &&
        switchOnDryRunOn.productionReleaseCrpFulfillClientAdapterHandoffReadinessSideEffectFree ===
          true &&
        switchOnDryRunOn.productionReleaseCrpFulfillClientAdapterHandoffReadiness?.safety
          ?.adapterInvoked === false &&
        switchOnDryRunOn.productionReleaseCrpFulfillClientAdapterHandoffReadiness?.safety
          ?.externalCallAttempted === false &&
        switchOnDryRunOn.productionReleaseCrpFulfillClientAdapterHandoffReadiness?.safety?.crpCalled ===
          false &&
        switchOnDryRunOn.productionReleaseCrpFulfillClientAdapterHandoffReadiness?.safety
          ?.crpFulfillCalled === false &&
        switchOnDryRunOn.productionReleaseCrpFulfillClientAdapterHandoffReadiness?.safety
  ...
  assert.equal(summary.crpFulfillClientAdapterHandoffReadinessObservedAfterReceipt, true);
  assert.equal(summary.crpFulfillClientAdapterHandoffReadinessPreservesReceiptContext, true);
  assert.equal(summary.crpFulfillClientAdapterHandoffReadinessBlocksExecution, true);
  assert.equal(summary.crpFulfillClientAdapterHandoffReadinessSideEffectFree, true);
