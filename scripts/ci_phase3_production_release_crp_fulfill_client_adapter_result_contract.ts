#!/usr/bin/env node
/**
 * PR #197 — Phase 3 production release CRP fulfill client adapter result contract boundary.
 *
 * This harness proves the Gateway exposes a stable CRP fulfill client adapter result
 * contract after handoff readiness, while keeping execution side-effect-free.
 *
...
const LABEL = "phase3:production-release-crp-fulfill-client-adapter-result-contract-test";

const SWITCH_OFF_GATEWAY_PORT = Number(process.env.PHASE3_PROD_CRP_FULFILL_CLIENT_ADAPTER_RESULT_CONTRACT_OFF_PORT || 3112);
const SWITCH_OFF_JWKS_PORT = Number(process.env.PHASE3_PROD_CRP_FULFILL_CLIENT_ADAPTER_RESULT_CONTRACT_OFF_JWKS_PORT || 8122);

const SWITCH_ON_GATEWAY_PORT = Number(process.env.PHASE3_PROD_CRP_FULFILL_CLIENT_ADAPTER_RESULT_CONTRACT_ON_PORT || 3113);
const SWITCH_ON_JWKS_PORT = Number(process.env.PHASE3_PROD_CRP_FULFILL_CLIENT_ADAPTER_RESULT_CONTRACT_ON_JWKS_PORT || 8123);
...
async function main() {
  ...
  const summary = {
    ok: true,
    harness: "phase3.productionReleaseCrpFulfillClientAdapterResultContract.v1",
