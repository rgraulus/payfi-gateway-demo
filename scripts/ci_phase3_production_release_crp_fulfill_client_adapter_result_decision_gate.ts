#!/usr/bin/env node
/**
 * PR #199 — Phase 3 production release CRP fulfill client adapter result decision gate boundary.
 *
 * This harness proves the Gateway observes the result-contract noop result,
 * then blocks CRP fulfill and production release through an explicit result decision gate.
 *
...
const LABEL = "phase3:production-release-crp-fulfill-client-adapter-result-decision-gate-test";

const SWITCH_OFF_GATEWAY_PORT = Number(process.env.PHASE3_PROD_CRP_FULFILL_CLIENT_ADAPTER_RESULT_DECISION_GATE_OFF_PORT || 3112);
const SWITCH_OFF_JWKS_PORT = Number(process.env.PHASE3_PROD_CRP_FULFILL_CLIENT_ADAPTER_RESULT_DECISION_GATE_OFF_JWKS_PORT || 8122);

const SWITCH_ON_GATEWAY_PORT = Number(process.env.PHASE3_PROD_CRP_FULFILL_CLIENT_ADAPTER_RESULT_DECISION_GATE_ON_PORT || 3113);
const SWITCH_ON_JWKS_PORT = Number(process.env.PHASE3_PROD_CRP_FULFILL_CLIENT_ADAPTER_RESULT_DECISION_GATE_ON_JWKS_PORT || 8123);
...
async function main() {
  ...
  const summary = {
    ok: true,
    harness: "phase3.productionReleaseCrpFulfillClientAdapterResultDecisionGate.v1",
