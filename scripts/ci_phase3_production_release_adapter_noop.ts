#!/usr/bin/env node
/**
 * PR #181 — Phase 3 production release adapter no-op function boundary.
 *
 * This harness proves the disabled/no-op adapter function boundary after the sanitized
 * adapter input is built.
 *
 * - switch OFF + dry-run OFF => no-op adapter result inactive
 * - switch OFF + dry-run ON  => no-op adapter result inactive
 * - switch ON  + dry-run OFF => no-op adapter result inactive
 * - switch ON  + dry-run ON  => no-op adapter result observed as disabled
 ...
 * - raw proof / receipt / JWS are not included
 */
...
const LABEL = "phase3:production-release-adapter-noop-test";

const SWITCH_OFF_GATEWAY_PORT = Number(process.env.PHASE3_PROD_ADAPTER_NOOP_OFF_PORT || 3104);
const SWITCH_OFF_JWKS_PORT = Number(process.env.PHASE3_PROD_ADAPTER_NOOP_OFF_JWKS_PORT || 8114);

const SWITCH_ON_GATEWAY_PORT = Number(process.env.PHASE3_PROD_ADAPTER_NOOP_ON_PORT || 3105);
const SWITCH_ON_JWKS_PORT = Number(process.env.PHASE3_PROD_ADAPTER_NOOP_ON_JWKS_PORT || 8115);
...
async function main() {
  ...
  const summary = {
    ok: true,
    harness: "phase3.productionReleaseAdapterNoop.v1",
