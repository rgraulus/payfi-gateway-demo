#!/usr/bin/env node
/**
 * PR #186 — Phase 3 production release CRP fulfill request validation boundary.
 *
 * This harness proves the Gateway validates the sanitized CRP fulfill request
 * draft after it is built, without sending the CRP fulfill request.
 *
...
const LABEL = "phase3:production-release-crp-fulfill-request-validation-test";

const SWITCH_OFF_GATEWAY_PORT = Number(process.env.PHASE3_PROD_CRP_FULFILL_VALIDATION_OFF_PORT || 3110);
const SWITCH_OFF_JWKS_PORT = Number(process.env.PHASE3_PROD_CRP_FULFILL_VALIDATION_OFF_JWKS_PORT || 8120);

const SWITCH_ON_GATEWAY_PORT = Number(process.env.PHASE3_PROD_CRP_FULFILL_VALIDATION_ON_PORT || 3111);
const SWITCH_ON_JWKS_PORT = Number(process.env.PHASE3_PROD_CRP_FULFILL_VALIDATION_ON_JWKS_PORT || 8121);
...
async function main() {
  ...
  const summary = {
    ok: true,
    harness: "phase3.productionReleaseCrpFulfillRequestValidation.v1",
