...
[[ -n "$NODE_EXE" ]] ||
  NODE_EXE="$(type -P node 2>/dev/null)"

[[ -n "$NODE_EXE" ]] || {
  echo "ERROR: Node executable was not found" >&2
  exit 1
}
...
WORKDIR="${WORKDIR:-.tmp/pr304-demo3-live-${RUN_ID}}"
...
PHASE6_DEMO3_PREFLIGHT_ONLY="${PHASE6_DEMO3_PREFLIGHT_ONLY:-true}"
...
DEMO3_MANIFEST_PATH="$WORKDIR/positive-controlled-evidence.json"
DEMO3_MANIFEST_HELPER_LOG="$WORKDIR/demo3-manifest-helper.json"
DEMO3_MANIFEST_HELPER_ERR="$WORKDIR/demo3-manifest-helper.err"
...
cleanup() {
  ...
  rm -f \
    "$DEMO3_MANIFEST_PATH" \
    "$DEMO3_MANIFEST_HELPER_LOG" \
    "$DEMO3_MANIFEST_HELPER_ERR" \
...
attach_agent_registry_reference() {
  local authorization_path="$1"

  "$NODE_EXE" \
    -r ts-node/register/transpile-only \
    - "$authorization_path" <<'NODE'
...
body.agentRegistryReference = {
  ...
  registryContract: {
    index:
      trusted.contract.index,

    subindex:
      trusted.contract.subindex,
  },

  agentTokenId,

  tokenAddress:
    `${trusted.network}/cis2:${trusted.contract.index}-${trusted.contract.subindex}-${agentTokenId}`,
};
...
NODE
  ...
  jq -e '
    (.agentRegistryReference.type | type == "string" and length > 0) and
    (.agentRegistryReference.version | type == "string" and length > 0) and
    .agentRegistryReference.registryStandard == "CIS-8004" and
    .agentRegistryReference.network == "ccd:testnet" and
    .agentRegistryReference.registryContract.index == "12802" and
    .agentRegistryReference.registryContract.subindex == 0 and
    .agentRegistryReference.agentTokenId == "0" and
    .agentRegistryReference.tokenAddress
      == "ccd:testnet/cis2:12802-0-0"
  ' "$authorization_path" >/dev/null ||
    fail "Agent Registry reference injection failed"
}

require_cmd curl
require_cmd npm

case "$PHASE6_DEMO3_PREFLIGHT_ONLY" in
  true|false)
    ;;

  *)
    fail "PHASE6_DEMO3_PREFLIGHT_ONLY must be exactly true or false"
    ;;
esac

[[ -f "node_modules/ts-node/dist/bin.js" ]] ||
  fail "Local ts-node executable was not found"

if curl -fsS "$GW/healthz" >/dev/null 2>&1; then
  fail "A Gateway is already reachable at $GW. Stop it before running the managed Demo3 autorun."
fi

say "Running live Concordium CIS-8004 token-0 smoke"

npm run \
  phase6:concordium-cis8004-registry-plugin-live-smoke

echo "Live token-0 registry smoke passed: true"
echo "Token-0 Agent Card positive evidence claimed: false"

say "Running controlled six-path PR #304 final acceptance"
...
echo "PR304 controlled six-path final acceptance passed: true"

if [[ "$PHASE6_DEMO3_PREFLIGHT_ONLY" == "true" ]]; then
  echo
  echo "============================================================"
  echo " Demo3 preflight complete"
  echo "  live CIS-8004 token-0 smoke: passed"
  echo "  controlled six-path Gateway acceptance: passed"
  echo "  managed live-settlement phase executed: false"
  ...
  echo "============================================================"
  echo
  echo "PR304_DEMO3_PREFLIGHT_COMPLETE=true"
  exit 0
fi

say "Entering explicitly enabled Demo3 live-settlement phase"
...
[[ -f "$WALLET_PATH" ]] ||
  fail "Wallet file not found at $WALLET_PATH"

[[ -f "$PHASE5_LIFECYCLE_MIGRATION" ]] ||
  fail "Phase 5 lifecycle migration was not found: $PHASE5_LIFECYCLE_MIGRATION"

echo "Demo3 live settlement explicitly enabled: true"
...
say "Generating temporary Phase 5 cryptographic keys"
...
then
  ...
  fail "Temporary Phase 5 cryptographic key generation failed"
fi
...
' "$KEY_BUNDLE_HELPER_LOG" >/dev/null || {
  ...
  sed 's/^/  /' "$KEY_BUNDLE_HELPER_LOG" >&2 || true
  fail "Temporary Phase 5 key helper returned an invalid result"
}
...
say "Generating controlled Demo3 positive registry evidence"

if ! npm exec -- ts-node --transpile-only \
  scripts/demo_phase6_demo3_controlled_evidence.ts \
  --key-bundle "$KEYDIR/phase5-cryptographic-key-bundle.json" \
  --scenario positive \
  --out "$DEMO3_MANIFEST_PATH" \
  > "$DEMO3_MANIFEST_HELPER_LOG" \
  2> "$DEMO3_MANIFEST_HELPER_ERR"
then
  echo
  echo "Demo3 controlled-evidence diagnostics:" >&2
  sed 's/^/  /' "$DEMO3_MANIFEST_HELPER_ERR" >&2 || true
  sed 's/^/  /' "$DEMO3_MANIFEST_HELPER_LOG" >&2 || true
  fail "Controlled Demo3 positive evidence generation failed"
fi

jq -e '
  .manifestValidated == true and
  .scenario == "positive" and
  .registryEvidenceControlled == true and
  .cis8EvidenceControlled == true and
  .agentCardEvidenceControlled == true and
  .actingKeyMatchesPhase5 == true and
  .deliveredAgentCardMatchesExpectedHash == true and
  .privateMaterialRead == false and
  .privateMaterialPrinted == false and
  .gatewayCalled == false and
  .concordiumCalled == false and
  .crpCalled == false and
  .paymentAttempted == false and
  .protectedResourceReleased == false
' "$DEMO3_MANIFEST_HELPER_LOG" >/dev/null ||
  fail "Controlled Demo3 evidence helper returned an invalid result"

[[ -s "$DEMO3_MANIFEST_PATH" ]] ||
  fail "Controlled Demo3 manifest was not generated"

echo "Controlled Demo3 positive manifest generated: true"
echo "Controlled registry fallback available: false"
echo "Private key material included in manifest: false"

say "Starting dedicated Demo3 Gateway"
...
PHASE3_LIVE_DIRECT_BUYER_CONTROLLED_RELEASE_DEMO_ENABLED=false \
...
PHASE6_DEMO3_CONTROLLED_EVIDENCE_ENABLED=true \
PHASE6_DEMO3_CONTROLLED_EVIDENCE_MANIFEST_PATH="$DEMO3_MANIFEST_PATH" \
  "$NODE_EXE" \
    node_modules/ts-node/dist/bin.js \
    src/server.ts \
    >"$GATEWAY_LOG" 2>&1 &
...
while (( $(date +%s) < deadline )); do
  ...
  if ! kill -0 "$GATEWAY_PID" >/dev/null 2>&1; then
    ...
    fail "Dedicated Demo3 Gateway failed during startup"
  fi
...
if ! curl -fsS "$GW/healthz" >/dev/null 2>&1; then
  ...
  sed 's/^/  /' "$GATEWAY_LOG" >&2 || true
  fail "Dedicated Demo3 Gateway readiness timed out"
fi
...
jq -e '
  ...
  .phase6.agentRegistryConditionalGatingEnabled == true and
  .phase6.agentRegistryConditionalGatingActive == true and
  .phase6.demo3ControlledEvidence.requested == true and
  .phase6.demo3ControlledEvidence.active == true and
  .phase6.demo3ControlledEvidence.status == "active" and
  .phase6.demo3ControlledEvidence.reason
    == "controlled_evidence_active" and
  .phase6.demo3ControlledEvidence.scenario == "positive" and
  .phase6.demo3ControlledEvidence.manifestLoaded == true and
  .phase6.demo3ControlledEvidence.manifestPathAccepted == true and
  .phase6.demo3ControlledEvidence.productionActivation == false and
  .phase6.productionActivation == false
' "$WORKDIR/gateway-health.json" >/dev/null || {
  echo
  echo "Unexpected Demo3 Gateway health contract:" >&2
  sed 's/^/  /' "$WORKDIR/gateway-health.json" >&2 || true
  fail "Dedicated Demo3 Gateway health contract failed"
}

echo
echo "============================================================"
echo " x402 v2 Agent Registry Demo3 — Live Settlement Phase"
echo " The controlled six-path PR #304 matrix already passed."
echo " This phase repeats four payment-safety paths against one"
echo " controlled positive registry manifest."
...
echo "   Expected: reject before policy; no payment or release"
echo " LIVE PATH 3 OF 4 - Authenticated agent, ineligible buyer"
echo "   Expected: policy denial before payment; no release"
echo " LIVE PATH 4 OF 4 - Authenticated agent, eligible buyer"
...
echo "  Delegation verification:  controlled cryptographic Phase 5"
echo "  Lifecycle enforcement:    enabled - PR #297"
echo "  Agent Registry lookup:    controlled Demo3 evidence"
echo "  Phase 5 production mode:  disabled"
echo "  Demo3 production mode:    disabled"
echo "  Live settlement enabled:  true"
...
echo "  Preflight only:           $PHASE6_DEMO3_PREFLIGHT_ONLY"
echo
...
echo "Per-request Agent Registry lookup pending at startup: true"
...
build_agent_delegated_auth() {
  ...
  attach_agent_registry_reference \
    "$WORKDIR/$prefix-agent-delegated-auth.json"

  echo "$prefix cryptographic delegation proof built: true"
  echo "$prefix Agent Registry reference attached: true"
}
...
say "LIVE PATH 1 OF 4 - Invalid buyer signature"
...
say "LIVE PATH 2 OF 4 - Invalid agent proof-of-possession"
...
say "LIVE PATH 3 OF 4 - Authenticated agent with ineligible buyer"
...
jq -e '
  .phase6.status == "allowed" and
  .phase6.reason == "accepted" and
  .phase6.auditPersisted == true and
  .phase6.agentRegistryLookupAttempted == true and
  .phase6.paymentEligibilityHandoffPresent == true and
  .phase6.productionActivation == false
' "$WORKDIR/negative-auth-body.json" >/dev/null ||
  fail "negative path expected Phase 6 authorization followed by buyer-policy denial"
...
say "LIVE PATH 4 OF 4 - Authenticated agent with eligible buyer"
...
jq -e '
  ...
  .phase6.productionActivation == false
' "$WORKDIR/positive-auth-body.json" >/dev/null ||
  fail "positive path expected Phase 6 authorization before payment eligibility"
...
echo "  LIVE PATH 1 - Invalid buyer signature: rejected before policy; no payment"
echo "  LIVE PATH 2 - Invalid agent proof: rejected before policy; no payment"
echo "  LIVE PATH 3 - Ineligible buyer: policy denied before payment"
echo "  LIVE PATH 4 - Eligible buyer: payment finalized and resource released"
echo "  Canonical state and transition evidence: captured"
echo "  Replay after release: blocked"
echo "  Controlled Agent Registry evidence: active"
echo "  Registry authorization audit: persisted"
echo "  Live token-0 smoke: passed separately"
echo "  Production activation: false"
echo "============================================================"
