#!/usr/bin/env node
/**
 * scripts/ci_phase3_live_direct_buyer_receipt_decode_readiness_preflight.ts
 *
 * Milestone 3G / PR #229 harness.
 *
 ...
 * is reduced to sanitized handoff metadata and a decode-readiness descriptor
 * bound to the fresh PAYMENT-REQUIRED tuple.
 *
 * This is intentionally a guarded local preflight harness. It does not submit
 * the receipt JWS to Gateway, does not build decoder input, does not invoke
 * decode, does not verify the receipt in the runtime path, does not release the
...
const GATEWAY_PORT = Number(process.env.PHASE3_LIVE_DIRECT_BUYER_RECEIPT_DECODE_READINESS_PREFLIGHT_PORT || 3102);
const JWKS_PORT = Number(process.env.PHASE3_LIVE_DIRECT_BUYER_RECEIPT_DECODE_READINESS_PREFLIGHT_JWKS_PORT || 8152);
const BASE = baseUrlForPort(GATEWAY_PORT);
const JWKS_BASE = baseUrlForPort(JWKS_PORT);
const LABEL = "phase3:live-direct-buyer-receipt-decode-readiness-preflight-test";
    ...
    JSON.stringify(
      {
        ...
        code: "usage",
        reason:
          "Usage: PHASE3_LIVE_DIRECT_BUYER_RECEIPT_DECODE_READINESS_PREFLIGHT_HARNESS=true npm run phase3:live-direct-buyer-receipt-decode-readiness-preflight-test -- <local-wallet-proof.json>",
...
function buildSanitizedReceiptJwsHandoff(input: {
  ...
  return {
    contract: "phase3.liveDirectBuyer.receiptDecodeReadinessPreflight.v1",
    mode: "preflight_only",
    status: "handoff_ready",
    reason: "receipt_decode_readiness_metadata_built_without_decoder_input",
    source: {
...
async function main() {
  ...
  if (String(process.env.PHASE3_LIVE_DIRECT_BUYER_RECEIPT_DECODE_READINESS_PREFLIGHT_HARNESS ?? "").toLowerCase() !== "true") {
    console.error(
      JSON.stringify(
        {
          ok: false,
          ...
          reason:
            "Set PHASE3_LIVE_DIRECT_BUYER_RECEIPT_DECODE_READINESS_PREFLIGHT_HARNESS=true to run this local-only harness.",
          rawProofPrinted: false,
          rawReceiptPrinted: false,
        },
        null,
  ...
  try {
    ...
    console.log(
      JSON.stringify(
        {
          ok: true,
          harness: "phase3.liveDirectBuyerReceiptDecodeReadinessPreflight.v1",
          ...
          decoderInvocationAllowed: handoff.decodeReadiness.decoderInvocationAllowed,
          decoderInvocationObserved: handoff.decodeReadiness.decoderInvocationObserved,
