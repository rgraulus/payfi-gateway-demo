#!/usr/bin/env node
/**
 * scripts/ci_phase3_live_direct_buyer_receipt_decoder_noop_to_real_readiness_preflight.ts
 *
 * Milestone 3H / PR #233 harness.
 *
 ...
 * guard, and a noop-to-real decoder readiness contract bound to the fresh
...
const GATEWAY_PORT = Number(process.env.PHASE3_LIVE_DIRECT_BUYER_RECEIPT_DECODER_NOOP_TO_REAL_READINESS_PREFLIGHT_PORT || 3105);
const JWKS_PORT = Number(process.env.PHASE3_LIVE_DIRECT_BUYER_RECEIPT_DECODER_NOOP_TO_REAL_READINESS_PREFLIGHT_JWKS_PORT || 8155);
const BASE = baseUrlForPort(GATEWAY_PORT);
const JWKS_BASE = baseUrlForPort(JWKS_PORT);
const LABEL = "phase3:live-direct-buyer-receipt-decoder-noop-to-real-readiness-preflight-test";
    ...
    JSON.stringify(
      {
        ...
        code: "usage",
        reason:
          "Usage: PHASE3_LIVE_DIRECT_BUYER_RECEIPT_DECODER_NOOP_TO_REAL_READINESS_PREFLIGHT_HARNESS=true npm run phase3:live-direct-buyer-receipt-decoder-noop-to-real-readiness-preflight-test -- <local-wallet-proof.json>",
...
async function main() {
  ...
  if (String(process.env.PHASE3_LIVE_DIRECT_BUYER_RECEIPT_DECODER_NOOP_TO_REAL_READINESS_PREFLIGHT_HARNESS ?? "").toLowerCase() !== "true") {
    console.error(
      JSON.stringify(
        {
          ok: false,
          ...
          reason:
            "Set PHASE3_LIVE_DIRECT_BUYER_RECEIPT_DECODER_NOOP_TO_REAL_READINESS_PREFLIGHT_HARNESS=true to run this local-only harness.",
          rawProofPrinted: false,
          rawReceiptPrinted: false,
        },
        null,
  ...
  try {
    ...
    console.log(
      JSON.stringify(
        {
          ok: true,
          harness: "phase3.liveDirectBuyerReceiptDecoderNoopToRealReadinessPreflight.v1",
