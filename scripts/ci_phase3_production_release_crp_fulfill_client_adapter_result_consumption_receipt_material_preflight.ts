#!/usr/bin/env node
/**
 * PR #211 — Phase 3 production release CRP fulfill client adapter result consumption receipt material preflight boundary.
 *
 * This harness proves the Gateway observes the noop consumer handling gate, then exposes
 * a receipt material preflight without consuming, decoding, or parsing receipt material.
 *
 ...
 * - switch ON  + dry-run ON  + result consumption ON  => receipt material preflight observed as ready and non-consuming
...
const LABEL = "phase3:production-release-crp-fulfill-client-adapter-result-consumption-receipt-material-preflight-test";

const SWITCH_OFF_GATEWAY_PORT = Number(process.env.PHASE3_PROD_CRP_FULFILL_CLIENT_ADAPTER_RESULT_CONSUMPTION_RECEIPT_MATERIAL_PREFLIGHT_OFF_PORT || 3132);
const SWITCH_OFF_JWKS_PORT = Number(process.env.PHASE3_PROD_CRP_FULFILL_CLIENT_ADAPTER_RESULT_CONSUMPTION_RECEIPT_MATERIAL_PREFLIGHT_OFF_JWKS_PORT || 8142);

const SWITCH_ON_GATEWAY_PORT = Number(process.env.PHASE3_PROD_CRP_FULFILL_CLIENT_ADAPTER_RESULT_CONSUMPTION_RECEIPT_MATERIAL_PREFLIGHT_ON_PORT || 3133);
const SWITCH_ON_JWKS_PORT = Number(process.env.PHASE3_PROD_CRP_FULFILL_CLIENT_ADAPTER_RESULT_CONSUMPTION_RECEIPT_MATERIAL_PREFLIGHT_ON_JWKS_PORT || 8143);
...
async function main() {
  ...
  const summary = {
    ok: true,
    harness: "phase3.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptMaterialPreflight.v1",
