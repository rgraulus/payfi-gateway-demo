#!/usr/bin/env node
/**
 * PR 4-11 — Phase 4 controlled real receipt release execution not-ready guard.
 *
 * This harness proves controlled execution fails closed when receipt settlement
 * is not release-ready:
 * - mock CRP fulfill is called once
 * - receipt JWS is handed off, decoded, and signature-verified
 * - settlement is intentionally non-finalized / not release-consumable
 * - release decision readiness remains false
 * - replay is not mutated
 * - canonical release is not persisted
...
const LABEL = "phase4:controlled-real-receipt-release-execution-not-ready-guard-test";
const GATEWAY_PORT = Number(process.env.PHASE4_CONTROLLED_REAL_RECEIPT_RELEASE_EXECUTION_NOT_READY_GUARD_PORT || 3132);
const CRP_PORT = Number(process.env.PHASE4_CONTROLLED_REAL_RECEIPT_RELEASE_EXECUTION_NOT_READY_GUARD_CRP_PORT || 8132);
const RECEIPT_KID = "phase4-test-only";
const RECEIPT_VERSION = "phase4.synthetic.controlled-release-execution-not-ready-guard.v1";
...
async function syntheticReceiptJws(body: any, privateKey: any): Promise<string> {
  ...
  return await new SignJWT({
    ...
    sub: "phase4-controlled-real-receipt-release-execution-not-ready-guard",
    ...
    settlement: {
      status: "pending",
      txHash: "phase4-synthetic-not-ready-guard-tx-hash",
    },
    testOnly: true,
...
async function main() {
  ...
  try {
    ...
    console.log(
      `[${LABEL}] not-ready response diagnostics: ${JSON.stringify(
        {
    ...
    assert.equal(release.status, 402, `not-ready receipt must not release resource: ${release.text}`);
    assert.equal(release.headers.get("payment-response"), null, "not-ready receipt must not emit PAYMENT-RESPONSE");
    assert.notEqual(release.json?.resource, "secret-data", "not-ready receipt must not release protected resource");
    assert.notEqual(release.json?.paid, true, "not-ready receipt must not report paid=true");

    assert.equal(fulfillRequests.length, 1, "not-ready guard should call mock CRP fulfill once");
    ...
    assert.equal(
      execution?.releaseDecisionPreflightStatus,
      "replay_canonical_persistence_preflight_not_observed",
    );
    assert.equal(settlementPreflight?.status, "not_finalized");
    assert.equal(settlementPreflight?.errorCode, "settlement_not_finalized");
    assert.equal(settlementPreflight?.settlement?.status, "pending");
    assert.equal(settlementPreflight?.settlement?.finalized, false);
    ...
    assert.equal(
      releaseDecisionPreflight?.status,
      "replay_canonical_persistence_preflight_not_observed",
    );
      ...
      JSON.stringify(
        {
          ...
          receiptReleaseConsumable: false,

          releaseAttemptStatus: release.status,
          notReadyBlocked: release.status === 402,
