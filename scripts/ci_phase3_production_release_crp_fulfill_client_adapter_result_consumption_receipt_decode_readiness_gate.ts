#!/usr/bin/env node
/**
 * PR #216 — Phase 3 production release CRP fulfill client adapter result consumption receipt decode readiness gate.
 *
 * This harness proves the Gateway observes the receipt decode dry-run audit, then exposes
 * a receipt decode readiness gate that blocks decode enablement while preserving side-effect safety.
 *
 ...
 * - switch ON  + dry-run ON  + result consumption ON  => receipt decode readiness gate observed as blocked and non-decoding
...
const LABEL = "phase3:production-release-crp-fulfill-client-adapter-result-consumption-receipt-decode-readiness-gate-test";

const SWITCH_OFF_GATEWAY_PORT = Number(process.env.PHASE3_PROD_CRP_FULFILL_CLIENT_ADAPTER_RESULT_CONSUMPTION_RECEIPT_DECODE_DRY_RUN_AUDIT_OFF_PORT || 3140);
const SWITCH_OFF_JWKS_PORT = Number(process.env.PHASE3_PROD_CRP_FULFILL_CLIENT_ADAPTER_RESULT_CONSUMPTION_RECEIPT_DECODE_DRY_RUN_AUDIT_OFF_JWKS_PORT || 8150);

const SWITCH_ON_GATEWAY_PORT = Number(process.env.PHASE3_PROD_CRP_FULFILL_CLIENT_ADAPTER_RESULT_CONSUMPTION_RECEIPT_DECODE_DRY_RUN_AUDIT_ON_PORT || 3141);
const SWITCH_ON_JWKS_PORT = Number(process.env.PHASE3_PROD_CRP_FULFILL_CLIENT_ADAPTER_RESULT_CONSUMPTION_RECEIPT_DECODE_DRY_RUN_AUDIT_ON_JWKS_PORT || 8151);
...
async function main() {
  ...
  const summary = {
    ok: true,
    harness: "phase3.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeReadinessGate.v1",
