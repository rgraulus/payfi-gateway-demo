#!/usr/bin/env node
/**
 * PR #204 — Phase 3 production release CRP fulfill client adapter result consumption handling gate boundary.
 *
 * This harness proves the Gateway observes the result consumption noop result,
 * then exposes a downstream blocked result consumption handling gate without consuming the result.
 *
...
const LABEL = "phase3:production-release-crp-fulfill-client-adapter-result-consumption-handling-gate-test";

const SWITCH_OFF_GATEWAY_PORT = Number(process.env.PHASE3_PROD_CRP_FULFILL_CLIENT_ADAPTER_RESULT_CONSUMPTION_HANDLING_GATE_OFF_PORT || 3118);
const SWITCH_OFF_JWKS_PORT = Number(process.env.PHASE3_PROD_CRP_FULFILL_CLIENT_ADAPTER_RESULT_CONSUMPTION_HANDLING_GATE_OFF_JWKS_PORT || 8128);

const SWITCH_ON_GATEWAY_PORT = Number(process.env.PHASE3_PROD_CRP_FULFILL_CLIENT_ADAPTER_RESULT_CONSUMPTION_HANDLING_GATE_ON_PORT || 3119);
const SWITCH_ON_JWKS_PORT = Number(process.env.PHASE3_PROD_CRP_FULFILL_CLIENT_ADAPTER_RESULT_CONSUMPTION_HANDLING_GATE_ON_JWKS_PORT || 8129);
...
async function main() {
  ...
  const summary = {
    ok: true,
    harness: "phase3.productionReleaseCrpFulfillClientAdapterResultConsumptionHandlingGate.v1",
