#!/usr/bin/env node
/**
 * PR #217 — Phase 3 production release CRP fulfill client adapter result consumption receipt decode enablement gate.
 *
 * This harness proves the Gateway exposes a receipt decode enablement gate after the
 * receipt decode readiness gate, while preserving non-execution and side-effect safety.
 *
 * - switch OFF + dry-run OFF => enablement gate inactive
 * - switch OFF + dry-run ON  => enablement gate inactive
 * - switch ON  + dry-run OFF => enablement gate inactive
 * - switch ON  + dry-run ON  + result consumption OFF => enablement gate inactive
 * - switch ON  + dry-run ON  + result consumption ON + receipt decode OFF => enablement gate blocked
 * - switch ON  + dry-run ON  + result consumption ON + receipt decode ON  => flag enabled, decode still unavailable
...
const LABEL = "phase3:production-release-crp-fulfill-client-adapter-result-consumption-receipt-decode-enablement-gate-test";

const SWITCH_OFF_GATEWAY_PORT = Number(process.env.PHASE3_PROD_CRP_FULFILL_CLIENT_ADAPTER_RESULT_CONSUMPTION_RECEIPT_DECODE_ENABLEMENT_GATE_OFF_PORT || 3142);
const SWITCH_OFF_JWKS_PORT = Number(process.env.PHASE3_PROD_CRP_FULFILL_CLIENT_ADAPTER_RESULT_CONSUMPTION_RECEIPT_DECODE_ENABLEMENT_GATE_OFF_JWKS_PORT || 8152);

const SWITCH_ON_GATEWAY_PORT = Number(process.env.PHASE3_PROD_CRP_FULFILL_CLIENT_ADAPTER_RESULT_CONSUMPTION_RECEIPT_DECODE_ENABLEMENT_GATE_ON_PORT || 3143);
const SWITCH_ON_JWKS_PORT = Number(process.env.PHASE3_PROD_CRP_FULFILL_CLIENT_ADAPTER_RESULT_CONSUMPTION_RECEIPT_DECODE_ENABLEMENT_GATE_ON_JWKS_PORT || 8153);
...
async function main() {
  ...
  const summary = {
    ok: true,
    harness: "phase3.productionReleaseCrpFulfillClientAdapterResultConsumptionReceiptDecodeEnablementGate.v1",
