#!/usr/bin/env node
/**
 * PR #198 — Phase 3 production release CRP fulfill client adapter result-contract noop result boundary.
 *
 * This harness proves the Gateway exposes a disabled noop result shaped by the
 * CRP fulfill client adapter result contract, while keeping execution side-effect-free.
 *
...
const LABEL = "phase3:production-release-crp-fulfill-client-adapter-result-contract-noop-result-test";

const SWITCH_OFF_GATEWAY_PORT = Number(process.env.PHASE3_PROD_CRP_FULFILL_CLIENT_ADAPTER_RESULT_CONTRACT_NOOP_RESULT_OFF_PORT || 3112);
const SWITCH_OFF_JWKS_PORT = Number(process.env.PHASE3_PROD_CRP_FULFILL_CLIENT_ADAPTER_RESULT_CONTRACT_NOOP_RESULT_OFF_JWKS_PORT || 8122);

const SWITCH_ON_GATEWAY_PORT = Number(process.env.PHASE3_PROD_CRP_FULFILL_CLIENT_ADAPTER_RESULT_CONTRACT_NOOP_RESULT_ON_PORT || 3113);
const SWITCH_ON_JWKS_PORT = Number(process.env.PHASE3_PROD_CRP_FULFILL_CLIENT_ADAPTER_RESULT_CONTRACT_NOOP_RESULT_ON_JWKS_PORT || 8123);
...
async function main() {
  ...
  const summary = {
    ok: true,
    harness: "phase3.productionReleaseCrpFulfillClientAdapterResultContractNoopResult.v1",
