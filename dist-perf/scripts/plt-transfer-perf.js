 ...
 */
function emitPerfLine(fields) {
    const parts = ["PERF_PAYER"];
    for (const [k, v] of Object.entries(fields)) {
        const val = typeof v === "boolean" ? (v ? "1" : "0") : typeof v === "number" ? String(v) : v;
...
async function main() {
    ...
    const grpcHost = args.grpcHost || "grpc.testnet.concordium.com";
    const grpcPort = Number(args.grpcPort || "20000");
    const wait = args["no-wait"] === true ? false : true; // default wait=true
    const memoStr = typeof args.memo === "string" ? args.memo : undefined;
    ...
    if (!(0, node_fs_1.existsSync)(walletPath)) {
    ...
    const { AccountAddress, parseWallet, buildAccountSigner, isKnown, TransactionSummaryType, TransactionKindString, RejectReasonTag, } = webSdk;
    ...
    const walletFile = (0, node_fs_1.readFileSync)(walletPath, "utf8");
    ...
    const tMemo0 = nowMs();
    if (memoStr) {
        const maybeCborMemo = pltSdk.CborMemo;
    ...
    const tMemo1 = nowMs();
    const transfer = { recipient, amount, memo };
    console.error(`[payer:plt:perf] tokenId=${tokenIdStr} decimals=${decimals} amount=${amountStr} to=${toAddr} sender=${walletExport.value.address} wait=${wait}`);
